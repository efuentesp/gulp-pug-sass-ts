...
(function($) {
  ...
  $.fn.alpha = function(settings) {
    ...
    var combinedSettings = getCombinedSettingsAlphaNum(
      settings,
      defaultAlphaSettings
    );
    ...
    $textboxes.each(function() {
      ...
      $textbox
        ...
        .on("keypress.alphanum", function(e) {
          ...
          var potentialTextAfterKeypress =
            textBeforeKeypress.substring(0, start) +
            newChar +
            textBeforeKeypress.substring(end);
          var validatedText = trimFunction(
  ...
  function getCombinedSettingsAlphaNum(settings, defaultSettings) {
    ...
    if (typeof combinedSettings.blacklist == "undefined")
      combinedSettings.blacklistSet = getBlacklistSet(
        combinedSettings.allow,
        combinedSettings.disallow
      );
  ...
  function alphanum_allowChar(validatedStringFragment, Char, settings) {
    if (
      settings.maxLength &&
      validatedStringFragment.length >= settings.maxLength
    )
      return false;
  ...
  function numeric_allowChar(validatedStringFragment, Char, settings) {
    ...
    if (
      Char == THOU_SEP &&
      settings.allowThouSep &&
      allowThouSep(validatedStringFragment)
    )
      return true;
  ...
  function allowThouSep(currentString) {
    ...
    var digitsSinceFirstThouSep = countDigits(
      currentString.substring(posOfFirstThouSep)
    );
