...
let destinationParams: UrlParams = {};
...
function manejoJson(datalist) {
  ...
  $("#totales").val("Ordenes sin Error: 0 / Ordenes con Error: 25");
}
...
$("#table_Ccapitales").jqGrid({
  ...
  colModel: [
    ...
    { name: "Digito", width: 80, align: "center" },
    { name: "Nombre", width: 80 },
    { name: "Emisora", width: 80 },
    { name: "Operacion", width: 80, align: "center" },
    { name: "Cantidad", width: 80, align: "right" },
    { name: "Precio", width: 80, align: "right" },
    { name: "Vigencia", width: 80, align: "center" },
    { name: "VO", width: 80 }
  ],
  gridComplete: function () {
    console.log("Context Menu");
    ($("tr.jqgrow", this) as any).contextMenu(
      "menu_ordenes",
      {
        bindings: {
          "personalizar": function (event) {
            console.log("Personalizar");
            $("#configColumns").dialog({
              modal: true,
              closeText: "",
              show: true,
              title: "Confirmación Columnas",
              width: 500
            })
          },
          "editOrden": function (event) {
            console.log("Editar Orden");
          },
          "copiOrden": function (event) {
            console.log("Copiar Orden");
          },
          "borrarOrden": function (event) {
            console.log("Borrar Orden");
          },
          "validaOrden": function (event) {
            console.log("Valida Orden");
          },
          "enviarOrden": function (event) {
            console.log("Enviar Orden");
          },
          "copiaExcel": function (event) {
...
function elementosContratos(dataC) {
  if (dataC != null && dataC != undefined) {
    document.getElementById("contrato").style.width = "10em";
    $("#field_digito").show();
    $("#field_titular").show();
    $("#field_perfilC").show();
    $("#field_IPContrato").show();
    ...
    document.getElementById("contrato").style.width = "17.5em";
    $("#field_titular").hide();
    $("#field_perfilC").hide();
    $("#field_IPContrato").hide();
    $("#field_digito").hide();
...
$("#btn_add").click(() => {
  mensajeEjecuta(1, "Aqui se agrega el mensaje del dialog");
});

$("#btn_send").click(() => {
});

const mensajeEjecuta = (opcion: number, mensaje: string) => {
...
const llenaSourceList = (id: string, source: any) => {
  fillSwapList(id, "source", source);
};

const llenaDestinationList = (id: string, destination: any) => {
