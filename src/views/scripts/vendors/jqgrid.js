...
(function(factory) {
  ...
  $.extend($.jgrid, {
    ...
    getXmlData: function(obj, expr, returnObj) {
      var ret,
        m = typeof expr === "string" ? expr.match(/^(.*)\[(\w+)\]$/) : null;
      if (typeof expr === "function") {
        return expr(obj);
      }
      if (m && m[2]) {
    ...
    cellWidth: function() {
      var $testDiv = $(
          "<div class='ui-jqgrid' style='left:10000px'><table class='ui-jqgrid-btable ui-common-table' style='width:5px;'><tr class='jqgrow'><td style='width:5px;display:block;'></td></tr></table></div>"
        ),
        testCell = $testDiv
          .appendTo("body")
          .find("td")
    ...
    getRegional: function(inst, param, def_val) {
      ...
      if (inst.p && inst.p.regional && $.jgrid.regional) {
        ret = $.jgrid.getAccessor(
          $.jgrid.regional[inst.p.regional] || {},
          param
        );
      }
      if (ret === undefined) {
        ...
        QueryObject = function(d, q) {
          ...
          this._getOrder = function(data, by, dir, type, dfmt, sfunc) {
            ...
            if (
              ...
              findSortKey = function($cell) {
                return $cell
                  ? parseFloat(String($cell).replace(_stripNum, ""))
              ...
              switch (swst) {
                ...
                case "integer":
                  val =
                    isNaN(Number(val)) || val === ""
                      ? Number.NEGATIVE_INFINITY
                      : val; // To be fixed with more inteligent code
                  fld = "parseInt(" + fld + ",10)";
                  val = "parseInt(" + val + ",10)";
                  break;
                case "float":
                ...
                case "numeric":
                  ...
                  val =
                    isNaN(Number(val)) || val === ""
                      ? Number.NEGATIVE_INFINITY
                      : Number(val); // To be fixed with more inteligent code
                  fld = "parseFloat(" + fld + ")";
                  val = "parseFloat(" + val + ")";
                  break;
                case "date":
                case "datetime":
                  val = String(
                    $.jgrid.parseDate
                      .call($t, t.srcfmt || "Y-m-d", val)
                      .getTime()
                  );
                  fld =
                    'jQuery.jgrid.parseDate.call(jQuery("#' +
                    $.jgrid.jqID($t.p.id) +
                    '")[0],"' +
                    t.srcfmt +
                    '",' +
          ...
          this.user = function(op, f, v) {
            self._append(
              "$t.p.customFilterDef." +
                op +
                '.action.call($t ,{rowItem:this, searchName:"' +
                f +
                '",searchValue:"' +
                v +
                '"})'
          ...
          this.inData = function(f, v, t) {
            var vl =
              v === undefined ? "" : self._getStr('"' + self._toStr(v) + '"');
            if (_useProperties) {
              self._append(
                vl +
                  ".split('" +
                  "," +
                  "')" +
                  ".indexOf( jQuery.jgrid.getAccessor(this,'" +
                  f +
                  "') ) > -1"
              );
            } else {
              self._append(
                vl + ".split('" + "," + "')" + ".indexOf(this." + f + ") > -1"
              );
            }
    ...
    gridUnload: function(jqGridId) {
      ...
      if ($t.p.pager) {
        $($t.p.pager)
          .off()
          .empty()
          .removeClass("ui-state-default ui-jqgrid-pager ui-corner-bottom");
      }
      var newtable = document.createElement("table");
      newtable.className = defgrid.cl;
      var gid = $.jgrid.jqID($t.id);
      $(newtable)
        .removeClass("ui-jqgrid-btable ui-common-table")
    ...
    getFont: function(instance) {
      ...
      return (
        getfont.getPropertyValue("font-style") +
        " " +
    ...
    return this.each(function() {
        ...
        grid = {
          ...
          dragMove: function(x) {
            if (this.resizing) {
              var diff = x.pageX - this.resizing.startX,
                ...
                newWidth =
                  p.direction === "ltr" ? h.width + diff : h.width - diff,
                hn,
                nWn;
              if (newWidth > this.resizing.minW) {
                ...
                if (p.forceFit === true) {
                  ...
                  nWn =
                    p.direction === "ltr" ? hn.width - diff : hn.width + diff;
          ...
          dragEnd: function(events) {
            ...
            if (this.resizing) {
              ...
              if (this.hrheaders.length > 0) {
                this.hrheaders[idx].style.width = nw + "px";
              }
              if (p.forceFit === true) {
                nw =
                  this.headers[idx + p.nv].newWidth ||
                ...
                if (this.hrheaders.length > 0) {
                  this.hrheaders[idx + p.nv].style.width = nw + "px";
                }
          ...
          populateVisible: function() {
            ...
            if (table[0].rows.length) {
              try {
                rows = table[0].rows[1];
                rh = rows
                  ? $(rows).outerHeight() || grid.prevRowHeight
                  : grid.prevRowHeight;
              } catch (pv) {
            ...
            if (
              tbot < dh &&
              ttop <= 0 &&
              (p.lastpage === undefined ||
                (parseInt((tbot + scrollTop + div - 1) / div, 10) || 0) <=
                  p.lastpage)
            ) {
            ...
            if (npage) {
              ...
              if (p.scrollPopUp && p.lastpage != null) {
                $("#scroll_g" + p.id)
                  ...
                  .html(
                    $.jgrid.template(
                      $.jgrid.getRegional(ts, "defaults.pgtext", p.pgtext),
                      p.page,
                      p.lastpage
                    )
                  )
                  .css({
                    top:
                      p.scrollTopOffset +
                      scrollTop *
                        ((parseInt(p.height, 10) - 45) /
                          (parseInt(rh, 10) * parseInt(p.records, 10))) +
                      "px",
      ...
      if (this.p.colNames.length === 0) {
        for (i = 0; i < this.p.colModel.length; i++) {
          this.p.colNames[i] =
            this.p.colModel[i].label || this.p.colModel[i].name;
        }
      }
      ...
      if (ts.p.styleUI === "Bootstrap4") {
        if ($.jgrid.iconSet.hasOwnProperty(ts.p.iconSet)) {
          $.extend(
            true,
            $.jgrid.styleUI["Bootstrap4"],
            $.jgrid.iconSet[ts.p.iconSet]
          );
        }
      }
      ...
      gv = $(
        "<div " +
          getstyle(stylemodule, "viewBox", false, "ui-jqgrid-view") +
          " role='grid'></div>"
      );
      ...
      var eg = $(
        "<div " +
          getstyle(stylemodule, "entrieBox", false, "ui-jqgrid") +
          "></div>"
      );
      $(eg)
        .attr({ id: "gbox_" + this.id, dir: dir })
        .insertBefore(gv);
      $(gv)
      ...
      $(
        "<div " +
          getstyle(stylemodule, "loadingBox", false, "loading") +
          " id='load_" +
          this.id +
          "'>" +
      ...
      var sortkeys = ["shiftKey", "altKey", "ctrlKey"],
        ...
        formatCol = function(pos, rowInd, tv, rawObject, rowId, rdata) {
          ...
          if (rowInd === 0) {
            ...
            (typeof cm.cellattr === "string" &&
              $.jgrid.cellattr != null &&
              $.isFunction($.jgrid.cellattr[cm.cellattr]))
          ) {
          ...
          result +=
            (clas !== undefined ? ' class="' + clas + '"' : "") +
            (cm.title && tv ? ' title="' + $.jgrid.stripHtml(tv) + '"' : "");
        ...
        formatter = function(rowId, cellval, colpos, rwdat, _act) {
          ...
          if (cm.formatter !== undefined) {
            rowId =
              String(ts.p.idPrefix) !== ""
                ? $.jgrid.stripPref(ts.p.idPrefix, rowId)
                : rowId;
            var opts = {
              rowId: rowId,
              colModel: cm,
        ...
        addRowNum = function(pos, irow, pG, rN, uiclass) {
          ...
          return (
            '<td role="gridcell" ' + uiclass + " " + prp + ">" + v + "</td>"
          );
        },
        ...
        normalizeData = function() {
          var p = ts.p,
            ...
            iOffset =
              (p.multiselect === true ? 1 : 0) +
              (p.subGrid === true ? 1 : 0) +
              (p.rownumbers === true ? 1 : 0),
          ...
          idn =
            p.keyName === false
              ? $.isFunction(localReader.id)
                ? localReader.id.call(ts, data)
                : localReader.id
              : p.keyName;
            ...
            if (idr === undefined) {
              ...
              if (idr === undefined) {
                ...
                if (cellName) {
                  ...
                  idr =
                    ccur != null && ccur[idn] !== undefined ? ccur[idn] : idr;
                  ccur = null;
                }
              }
            }
        ...
        constructTr = function(id, hide, classes, rd, cur) {
          ...
          if (typeof rowAttrObj !== "object") {
            rowAttrObj = $.isFunction(ts.p.rowattr)
              ? ts.p.rowattr.call(ts, rd, cur, id)
              : typeof ts.p.rowattr === "string" &&
                $.jgrid.rowattr != null &&
                $.isFunction($.jgrid.rowattr[ts.p.rowattr])
        ...
        treeGrid_beforeRequest = function() {
          if (ts.p.treeGrid && ts.p.treeGrid_bigData) {
            if (
              ts.p.postData.nodeid !== undefined &&
        ...
        treeGrid_afterLoadComplete = function() {
          if (ts.p.treeGrid && ts.p.treeGrid_bigData) {
            if (
              ts.p.treeGrid_rootParams.otherData.nodeid !== undefined &&
              ...
              if (
                ts.p.treeGrid_rootParams !== undefined &&
                ts.p.treeGrid_rootParams != null
              ) {
                ...
                ts.p.postData.totalrows =
                  ts.p.treeGrid_rootParams.postData.totalrows;
        ...
        addXmlData = function(xml, rcnt, more, adjust) {
          ...
          if (gxml && gl) {
            ...
            var afterInsRow = $.isFunction(ts.p.afterInsertRow),
              rnc = ni
                ? getstyle(stylemodule, "rownumBox", false, "jqgrid-rownum")
                : "",
            ...
            while (j < gl) {
              ...
              if (xmlRd.repeatitems) {
                ...
                $.each(F, function(k) {
                  ...
                  rowData.push(
                    addCell(rid, v, k + gi + si + ni, j + rcnt, xmlr, rd)
                  );
                });
              } else {
                for (i = 0; i < f.length; i++) {
                  ...
                  rowData.push(
                    addCell(rid, v, i + gi + si + ni, j + rcnt, xmlr, rd)
                  );
                }
              }
              rowData[iStartTrTag] = constructTr(
                rid,
                hiderow,
                classes,
                rd,
          ...
          if (ts.p.userDataOnFooter) {
            self.jqGrid("footerData", "set", ts.p.userData, false);
          }
          if (ts.p.userDataOnHeader) {
            self.jqGrid("headerData", "set", ts.p.userData, false);
          }
          if (locdata) {
            ts.p.records = gl;
        ...
        addJSONData = function(data, rcnt, more, adjust) {
          ...
          var locid = "_id_",
            ...
            ni = ts.p.rownumbers === true ? 1 : 0,
            br =
              ts.p.scroll && ts.p.datatype !== "local" ? $.jgrid.randId() : 1,
            rn = parseInt(ts.p.rowNum, 10),
            selected = false,
            selr,
            ...
            afterInsRow = $.isFunction(ts.p.afterInsertRow),
            grpdata = [],
            hiderow = false,
            groupingPrepare,
            tablebody = $(ts).find("tbody:first"),
          ...
          ts.p.reccount = 0;
          ts.p.page = intNum(
            $.jgrid.getAccessor(data, dReader.page),
            ts.p.page
          );
          ...
          if (ts.p.keyName === false) {
            idn = $.isFunction(dReader.id)
              ? dReader.id.call(ts, data)
              : dReader.id;
          } else {
            idn = ts.p.keyName;
          }
          ...
          while (i < len) {
            ...
            if (idr === undefined) {
              if (
                typeof idn === "number" &&
                ts.p.colModel[idn + gi + si + ni] != null
              ) {
                // reread id by name
                idr = $.jgrid.getAccessor(
                  cur,
                  ts.p.colModel[idn + gi + si + ni].name
                );
              }
              if (idr === undefined) {
                idr = br + i;
            ...
            for (j = 0; j < rowReader.length; j++) {
              ...
              rowData.push(
                addCell(idr, v, j + gi + si + ni, i + rcnt, cur, rd)
              );
            }
            rowData[iStartTrTag] = constructTr(
              idr,
              hiderow,
              selr ? classes + " " + highlight : classes,
              rd,
              cur
            );
            rowData.push("</tr>");
          ...
          spsh =
            ts.p.multiselect &&
            (ts.p.preserveSelection || selected) &&
            ir === spsh;
          if (ts.p.gridview === true) {
            fpos = ts.p.treeANode > -1 ? ts.p.treeANode : 0;
            if (ts.p.grouping) {
          ...
          if (locdata) {
            while (ir < len && drows[ir]) {
                ...
                if (idr === undefined) {
                  ...
                  if (f.length === 0) {
                    if (dReader.cell) {
                      ...
                      idr =
                        ccur2 != null && ccur2[idn] !== undefined
                          ? ccur2[idn]
                          : idr;
                      ccur2 = null;
                    }
              ...
              if (cur) {
                ...
                for (j = 0; j < rowReader.length; j++) {
                  rd[
                    ts.p.colModel[j + gi + si + ni].name
                  ] = $.jgrid.getAccessor(cur, rowReader[j]);
                }
                rd[locid] = $.jgrid.stripPref(ts.p.idPrefix, idr);
                if (ts.p.grouping) {
                  groupingPrepare.call(self, rd, ir);
        ...
        addLocalData = function(retAll) {
          ...
          $.each(ts.p.colModel, function() {
            ...
            if (sorttype === "date" || sorttype === "datetime") {
              if (
                ...
                if (this.formatoptions && this.formatoptions.srcformat) {
                  ...
                  srcformat = $.jgrid.getRegional(
                    ts,
                    "formatter.date.srcformat"
                  );
                }
                if (this.formatoptions && this.formatoptions.newformat) {
                  newformat = this.formatoptions.newformat;
                } else {
                  newformat = $.jgrid.getRegional(
                    ts,
                    "formatter.date.newformat"
                  );
                }
              } else {
            ...
            if (ts.p.grouping) {
              for (
                gin = 0, lengrp = grpview.groupField.length;
                gin < lengrp;
                gin++
              ) {
          ...
          var compareFnMap = {
              ...
              ni: function(queryObj, op) {
                return op === "OR"
                  ? queryObj.orNot().inData
                  : queryObj.andNot().inData;
              },
          ...
          function tojLinq(group) {
            ...
            if (group.groups != null) {
              gor =
                group.groups.length &&
                group.groupOp.toString().toUpperCase() === "OR";
              if (gor) {
                query.orBegin();
              }
              ...
              try {
                ...
                for (index = 0; index < group.rules.length; index++) {
                  ...
                  if (compareFnMap[rule.op] && rule.field) {
                    ...
                    try {
                      ...
                      if (fld.stype === "date") {
                        if (
                          fld.srcfmt &&
                          fld.newfmt &&
                          fld.srcfmt !== fld.newfmt
                        ) {
                          rule.data = $.jgrid.parseDate.call(
                            ts,
                            fld.newfmt,
                            rule.data,
                            fld.srcfmt
                    ...
                    ts.p.customFilterDef !== undefined &&
                    ts.p.customFilterDef[rule.op] !== undefined &&
                    $.isFunction(ts.p.customFilterDef[rule.op].action)
                  ) {
              ...
              try {
                ...
                if (sfld.stype === "date") {
                  if (
                    sfld.srcfmt &&
                    sfld.newfmt &&
                    sfld.srcfmt !== sfld.newfmt
                  ) {
                    ts.p.postData.searchString = $.jgrid.parseDate.call(
                      ts,
                      sfld.newfmt,
                ...
                if (compareFnMap[ts.p.postData.searchOper]) {
                  ...
                  ts.p.customFilterDef !== undefined &&
                  ts.p.customFilterDef[ts.p.postData.searchOper] !==
                    undefined &&
                  $.isFunction(
                    ts.p.customFilterDef[ts.p.postData.searchOper].action
                  )
                ) {
                  query = query.user.call(
                    ts,
                    ts.p.postData.searchOper,
                    ts.p.postData.searchField,
                    ts.p.postData.searchString
                  );
                }
          ...
          var queryResults = query.select(),
            ...
            retresult = {};
          if (
            (ts.p.search || ts.p.resetsearch) &&
            ts.p.grouping &&
            ts.p.groupingView._locgr
          ) {
            ts.p.groupingView.groups = [];
            var j,
              grPrepare = $.jgrid.getMethod("groupingPrepare"),
              key,
              udc;
        ...
        updatepager = function(rn, dnd) {
          ...
          if (ts.p.scroll) {
            ...
            if (rh) {
              ...
              $(">div:first", ts.grid.bDiv)
                .css({ height: height })
                .children("div:first")
                .css({ height: top, display: top ? "" : "none" });
          ...
          if (pgboxes) {
            ...
            $(".selbox", pgboxes)[this.p.useProp ? "prop" : "attr"](
              "disabled",
              false
            );
            if (ts.p.pginput === true) {
              $("#input" + tspg).html(
                $.jgrid.template(
                  $.jgrid.getRegional(ts, "defaults.pgtext", ts.p.pgtext) || "",
                  "<input " +
                    getstyle(stylemodule, "pgInput", false, "ui-pg-input") +
                    " type='text' size='2' maxlength='7' value='0' role='textbox'/>",
                  "<span id='sp_1_" + $.jgrid.jqID(pgid) + "'></span>"
                )
              );
              if (ts.p.toppager) {
                $("#input_t" + tspg_t).html(
              ...
              sppg = ts.p.toppager
                ? "#sp_1" + tspg + ",#sp_1" + tspg + "_toppager"
                : "#sp_1" + tspg;
              $(sppg).html(
                $.fmatter
                  ? $.fmatter.util.NumberFormat(ts.p.lastpage, fmt)
                  : ts.p.lastpage
              );
            }
            if (ts.p.viewrecords) {
              if (ts.p.reccount === 0) {
                ...
                var rt = $.jgrid.getRegional(
                  ts,
                  "defaults.recordtext",
                  ts.p.recordtext
                );
                $(".ui-paging-info", pgboxes).html(
                  $.jgrid.template(rt, from, to, tot)
                );
              }
            }
            if (ts.p.pgbuttons === true) {
              ...
              if (cp === 1 || cp === 0) {
                ...
                if (ts.p.toppager) {
                  $("#first_t" + tspg_t + ", #prev_t" + tspg_t).removeClass(
                    disabled
                  );
                }
              }
              if (cp === last || cp === 0) {
                ...
                if (ts.p.toppager) {
                  $("#next_t" + tspg_t + ", #last_t" + tspg_t).removeClass(
                    disabled
                  );
                }
              }
        ...
        beforeprocess = function(data, st, xhr) {
          var bfpcr = $(ts).triggerHandler("jqGridBeforeProcessing", [
            data,
            st,
            xhr
          ]);
          bfpcr =
            bfpcr === undefined || typeof bfpcr !== "boolean" ? true : bfpcr;
          ...
          if (!ts.grid.hDiv.loading) {
            ...
            switch (dt) {
                  ...
                  $.extend(
                    {
                      ...
                      beforeSend: function(xhr, settings) {
                        var gotoreq = true;
                        gotoreq = $(ts).triggerHandler("jqGridLoadBeforeSend", [
                          xhr,
                          settings
                        ]);
              ...
              case "xmlstring":
                beginReq();
                dstr =
                  typeof ts.p.datastr !== "string"
                    ? ts.p.datastr
                    : $.parseXML(ts.p.datastr);
        ...
        setHeadCheckBox = function(checked) {
          ...
          if (fid) {
            $("#cb_" + $.jgrid.jqID(ts.p.id), ts.grid.fhDiv)[
              ts.p.useProp ? "prop" : "attr"
            ]("checked", checked);
          }
        },
        setPager = function(pgid, tp) {
          ...
          var sep =
            ...
            pginp = "",
            pgl =
              "<table class='ui-pg-table ui-common-table ui-paging-pager'><tbody><tr>",
            str = "",
            pgcnt,
            lft,
            cent,
            ...
            clearVals = function(onpaging, thus) {
              ...
              if ($.isFunction(ts.p.onPaging)) {
                ret = ts.p.onPaging.call(ts, onpaging, thus);
              }
              if (ret === "stop") {
                return false;
              }
          ...
          $("#" + $.jgrid.jqID(pgid))
            .append(
              "<div id='" +
                pgcnt +
                "' class='ui-pager-control' role='group'><table " +
                getstyle(
                  stylemodule,
                  "pagerTable",
                  false,
          ...
          if (ts.p.rowList.length > 0) {
            ...
            str +=
              "<select " +
              getstyle(stylemodule, "pgSelectBox", false, "ui-pg-selbox") +
              ' size="1" role="listbox" title="' +
              ($.jgrid.getRegional(ts, "defaults.pgrecs", ts.p.pgrecs) || "") +
              '">';
            var strnm;
            for (i = 0; i < ts.p.rowList.length; i++) {
              ...
              str +=
                ...
                strnm[0] +
                '"' +
                (intNum(ts.p.rowNum, 0) === intNum(strnm[0], 0)
                  ? ' selected="selected"'
                  : "") +
                ">" +
                strnm[1] +
          ...
          if (ts.p.pgbuttons === true) {
            var po = ["first" + tp, "prev" + tp, "next" + tp, "last" + tp],
              btc = getstyle(stylemodule, "pgButtonBox", true, "ui-pg-button"),
              pot = [
                $.jgrid.getRegional(ts, "defaults.pgfirst", ts.p.pgfirst) || "",
            ...
            pgl +=
              ...
              "'" +
              "><span " +
              getstyle(stylemodule, "icon_first", false, iconbase) +
              "></span></td>";
            pgl +=
              "<td id='" +
              po[1] +
              ...
              "'" +
              "><span " +
              getstyle(stylemodule, "icon_prev", false, iconbase) +
              "></span></td>";
            ...
            pgl +=
              ...
              "'" +
              "><span " +
              getstyle(stylemodule, "icon_next", false, iconbase) +
              "></span></td>";
            pgl +=
              "<td id='" +
              po[3] +
              ...
              "'" +
              "><span " +
              getstyle(stylemodule, "icon_end", false, iconbase) +
              "></span></td>";
          } else if (pginp !== "") {
            pgl += pginp;
          }
          ...
          if (ts.p.viewrecords === true) {
            $("td#" + pgid + "_" + ts.p.recordpos, "#" + pgcnt).append(
              "<div dir='" +
                dir +
                "' style='text-align:" +
                ts.p.recordpos +
          ...
          if (twd > 0) {
            ...
            removebutt =
              twd >
              $("td#" + pgid + "_" + ts.p.pagerpos, "#" + pgcnt).innerWidth();
            $("td#" + pgid + "_" + ts.p.pagerpos, "#" + pgcnt).width(twd);
          }
          ts.p._nvtd = [];
          ts.p._nvtd[0] = twd
            ? Math.floor((ts.p.width - twd) / 2)
            : Math.floor(ts.p.width / 3);
          ts.p._nvtd[1] = 0;
          pgl = null;
          $(".ui-pg-selbox", "#" + pgcnt).on("change", function() {
            if (!clearVals("records", this)) {
              return false;
            }
            ts.p.page =
              Math.round((ts.p.rowNum * (ts.p.page - 1)) / this.value - 0.5) +
              1;
          ...
          if (ts.p.pgbuttons === true) {
            ...
            $(
              ...
              if (!clearVals(this.id.split("_")[0], this)) {
                return false;
              }
          ...
          if (ts.p.pginput === true) {
            $("#" + pgcnt).on("keypress", "input.ui-pg-input", function(e) {
              ...
              if (key === 13) {
                if (!clearVals("user", this)) {
                  return false;
                }
                $(this).val(intNum($(this).val(), 1));
                ts.p.page =
                  $(this).val() > 0 && $(this).val() <= ts.p.lastpage
                    ? $(this).val()
                    : ts.p.page;
                populate();
                return false;
          ...
          if (typeof sor == "undefined") {
            if (cm[iCol].lso) {
              if (cm[iCol].lso === "asc") {
                ...
                cm[iCol].lso === "asc-desc" ||
                cm[iCol].lso === "desc-asc"
              ) {
                cm[iCol].lso = "";
              }
            } else {
      ...
      */
        ...
        sortData = function(index, idxcol, reload, sor, obj) {
          ...
          if (ts.p.multiSort) {
            ...
            var previousSelectedTh = ts.grid.headers[ts.p.lastsort]
                ? ts.grid.headers[ts.p.lastsort].el
                : null,
              newSelectedTh = ts.p.frozenColumns
                ? obj
                : ts.grid.headers[idxcol].el,
              //sortrule = $.trim(ts.p.viewsortcols[1] === 'single' ? hidden : disabled);
              usehide = ts.p.viewsortcols[1] === "single" ? true : false,
              tmpicon;
            ...
            if (!ts.p.viewsortcols[0]) {
              ...
              } else if (ts.p.sortname === "") {
                // if ts.p.lastsort === idxcol but ts.p.sortname === ""
                $("span.s-ico", newSelectedTh).show();
              }
            }
          ...
          if (
            $(ts).triggerHandler("jqGridSortCol", [
              ts.p.sortname,
              idxcol,
              ts.p.sortorder
            ]) === "stop"
          ...
          if ($.isFunction(ts.p.onSortCol)) {
            if (
              ts.p.onSortCol.call(ts, ts.p.sortname, idxcol, ts.p.sortorder) ===
              "stop"
            ) {
              ts.p.lastsort = idxcol;
              return;
        ...
        setColWidth = function() {
          ...
          if (isNaN(ts.p.width)) {
            ts.p.width =
              initwidth +
              (ts.p.shrinkToFit === false && !isNaN(ts.p.height) ? scw : 0);
          }
          ...
          if (ts.p.shrinkToFit === true && vc > 0) {
            ...
            $.each(ts.p.colModel, function(i) {
              if (this.hidden === false && !this.fixed) {
                cw = Math.round(
                  (aw * this.width) / (ts.p.tblwidth - brd * vc - gw)
                );
                this.width = cw;
                initwidth += cw;
                lvc = i;
            ...
            if (hs) {
              ...
              !hs &&
              Math.abs(grid.width - gw - (initwidth + brd * vc)) !== 0
            ) {
            ...
            if (ts.p.tblwidth > ts.p.width) {
              ts.p.colModel[lvc].width -=
                ts.p.tblwidth - parseInt(ts.p.width, 10);
        ...
        getOffset = function(iCol) {
          ...
          ret.push(
            $(ts.grid.bDiv).offset().top -
              $(ts.grid.hDiv).offset().top +
              $(ts.grid.bDiv).height()
          );
          return ret;
        },
        ...
        buildColItems = function(top, left, parent) {
          var cm = ts.p.colModel,
            ...
            str1 =
              '<ul id="col_menu" class="ui-search-menu  ui-col-menu modal-content" role="menu" tabindex="0" style="left:' +
              left +
              'px;">';
          for (i = 0; i < len; i++) {
            ...
            var hid = !cm[i].hidden ? "checked" : "",
              ...
              lb = ts.p.colNames[i];
            disp =
              nm === "cb" || nm === "subgrid" || nm === "rn" || cm[i].hidedlg
                ? "style='display:none'"
                : "";
            str1 +=
              "<li " +
              disp +
              ' class="ui-menu-item" role="presentation" draggable="true"><a class="g-menu-item" tabindex="0" role="menuitem" ><table class="ui-common-table" ><tr><td class="menu_icon" title="' +
              texts.reorder +
              '"><span class="' +
              iconbase +
              " " +
              colmenustyle.icon_move +
              ' notclick"></span></td><td class="menu_icon"><input class="' +
              colmenustyle.input_checkbox +
              '" type="checkbox" name="' +
              nm +
              '" ' +
              hid +
          ...
          if (!$.jgrid.isElementInViewport($("#col_menu")[0])) {
            $("#col_menu").css(
              "left",
              -parseInt($("#column_menu").innerWidth(), 10) + "px"
            );
          }
          if ($.fn.html5sortable()) {
            $("#col_menu")
              ...
              .on("sortupdate", function(e, ui) {
                ...
                $(ts).triggerHandler("jqGridColMenuColumnDone", [
                  cols,
                  null,
                  null
                ]);
        ...
        buildSearchBox = function(index, top, left, parent) {
          ...
          rules =
            ts.p.colFilters && ts.p.colFilters[cm.name]
          ...
          elem = $("<form></form>");
          var str1 =
            "<div>" + $.jgrid.getRegional(ts, "colmenu.searchTitle") + "</div>";
          str1 +=
            '<div><select size="1" id="oper1" class="' +
            colmenustyle.filter_select +
            '">';
          ...
          if (cm.searchoptions.defaultValue) {
            df = $.isFunction(cm.searchoptions.defaultValue)
              ? cm.searchoptions.defaultValue.call(ts)
              : cm.searchoptions.defaultValue;
          }
          //overwrite default value if restore from filters
          if (v1) {
            df = v1;
          }
          var soptions = $.extend(cm.searchoptions, {
              name: cm.index || cm.name,
              id: "sval1_" + ts.p.idPrefix + cm.name,
              oper: "search"
            }),
            input = $.jgrid.createEl.call(
              ts,
              cm.stype,
              soptions,
              df,
              false,
              $.extend({}, $.jgrid.ajaxOptions, ts.p.ajaxSelectOptions || {})
            );
          ...
          // and/or
          str1 =
            '<div><select size="1" id="operand" class="' +
            colmenustyle.filter_select +
            '">';
          ...
          str1 =
            '<div><select size="1" id="oper2" class="' +
            colmenustyle.filter_select +
            '">' +
            repstr +
            "</select></div>";
          ...
          soptions = $.extend(cm.searchoptions, {
            name: cm.index || cm.name,
            id: "sval2_" + ts.p.idPrefix + cm.name,
            oper: "search"
          });
          input = $.jgrid.createEl.call(
            ts,
          ...
          str1 = "<div>";
          str1 +=
            "<div class='search_buttons'><a tabindex='0' id='bs_reset' class='fm-button " +
            common.button +
            " ui-reset'>" +
            texts.Reset +
            "</a></div>";
          str1 +=
            "<div class='search_buttons'><a tabindex='0' id='bs_search' class='fm-button " +
            common.button +
            " ui-search'>" +
            texts.Find +
            "</a></div>";
          str1 += "</div>";
          elem.append(str1);
          elem = $('<li class="ui-menu-item" role="presentation"></li>').append(
            elem
          );
          elem = $(
            '<ul id="search_menu" class="ui-search-menu modal-content" role="menu" tabindex="0" style="left:' +
              left +
              'px;"></ul>'
          ).append(elem);
          $(parent).append(elem);
          ...
          $("#bs_reset, #bs_search", "#search_menu").hover(
            function() {
              $(this).addClass(hover);
            },
            function() {
          ...
          $("#bs_search", elem).on("click", function(e) {
            ts.p.colFilters[cm.name] = {
              ...
              value1: $(
                "#sval1_" + ts.p.idPrefix + cm.name,
                "#search_menu"
              ).val(),
              rule: $("#operand", "#search_menu").val(),
              oper2: $("#oper2", "#search_menu").val(),
              value2: $(
                "#sval2_" + ts.p.idPrefix + cm.name,
                "#search_menu"
              ).val()
            };
            ts.p.postData.filters = buildFilters();
        ...
        buildFilters = function() {
          var go = "AND",
            filters = '{"groupOp":"' + go + '","rules":[], "groups" : [',
            i = 0;
        ...
        buildColMenu = function(index, left, top) {
          ...
          if (
            $(".ui-search-toolbar", grid.hDiv)[0] &&
            !isNaN($(".ui-search-toolbar", grid.hDiv).height())
          ) {
          ...
          if (op.sorting) {
            str =
              '<li class="ui-menu-item" role="presentation"><a class="g-menu-item" tabindex="0" role="menuitem" data-value="sortasc"><table class="ui-common-table"><tr><td class="menu_icon"><span class="' +
              iconbase +
              " " +
              colmenustyle.icon_sort_asc +
              '"></span></td><td class="menu_text">' +
              texts.sortasc +
              "</td></tr></table></a></li>";
            str +=
              '<li class="ui-menu-item" role="presentation"><a class="g-menu-item" tabindex="0" role="menuitem" data-value="sortdesc"><table class="ui-common-table"><tr><td class="menu_icon"><span class="' +
              iconbase +
              " " +
              colmenustyle.icon_sort_desc +
              '"></span></td><td class="menu_text">' +
              texts.sortdesc +
              "</td></tr></table></a></li>";
            menuData.push(str);
          ...
          if (op.columns) {
            ...
            str +=
              '<li class="ui-menu-item" role="presentation"><a class="g-menu-item" tabindex="0" role="menuitem" data-value="columns"><table class="ui-common-table"><tr><td class="menu_icon"><span class="' +
              iconbase +
              " " +
              colmenustyle.icon_columns +
              '"></span></td><td class="menu_text">' +
              texts.columns +
              "</td></tr></table></a></li>";
            menuData.push(str);
          ...
          if (op.filtering) {
            str = '<li class="ui-menu-item divider" role="separator"></li>';
            str +=
              '<li class="ui-menu-item" role="presentation"><a class="g-menu-item" tabindex="0" role="menuitem" data-value="filtering"><table class="ui-common-table"><tr><td class="menu_icon"><span class="' +
              iconbase +
              " " +
              colmenustyle.icon_filter +
              '"></span></td><td class="menu_text">' +
              texts.filter +
              " " +
              label +
          ...
          if (op.grouping) {
            ...
            str = '<li class="ui-menu-item divider" role="separator"></li>';
            str +=
              '<li class="ui-menu-item" role="presentation"><a class="g-menu-item" tabindex="0" role="menuitem" data-value="grouping"><table class="ui-common-table"><tr><td class="menu_icon"><span class="' +
              iconbase +
              " " +
              colmenustyle.icon_group +
              '"></span></td><td class="menu_text">' +
              (isgroup !== -1
                ? texts.ungrouping
                : texts.grouping + " " + label) +
              "</td></tr></table></a></li>";
            menuData.push(str);
          }
          if (op.freeze) {
            ...
            str = '<li class="ui-menu-item divider" role="separator"></li>';
            str +=
              '<li class="ui-menu-item" role="presentation"><a class="g-menu-item" tabindex="0" role="menuitem" data-value="freeze"><table class="ui-common-table"><tr><td class="menu_icon"><span class="' +
              iconbase +
              " " +
              colmenustyle.icon_freeze +
              '"></span></td><td class="menu_text">' +
              (isfreeze ? texts.freeze + " " + label : texts.unfreeze) +
              "</td></tr></table></a></li>";
            menuData.push(str);
          }
          for (var key in ts.p.colMenuCustom) {
            if (ts.p.colMenuCustom.hasOwnProperty(key)) {
              ...
              exclude = $.map(exclude, function(item) {
                return $.trim(item);
              });
              if (
                menuitem.colname === cname ||
                (menuitem.colname === "_all_" &&
                  $.inArray(cname, exclude) === -1)
              ) {
                strl =
                  '<li class="ui-menu-item divider" role="separator"></li>';
                str =
                  '<li class="ui-menu-item" role="presentation"><a class="g-menu-item" tabindex="0" role="menuitem" data-value="' +
                  menuitem.id +
                  '"><table class="ui-common-table"><tr><td class="menu_icon"><span class="' +
                  iconbase +
                  " " +
                  menuitem.icon +
                  '"></span></td><td class="menu_text">' +
          ...
          $("#column_menu")
            .addClass(
              "ui-search-menu modal-content column-menu jqgrid-column-menu ui-menu " +
                colmenustyle.menu_widget
            )
      ...
      for (i = 0; i < ts.p.colModel.length; i++) {
        colTemplate =
          typeof ts.p.colModel[i].template === "string"
            ? $.jgrid.cmTemplate != null &&
              typeof $.jgrid.cmTemplate[ts.p.colModel[i].template] === "object"
              ? $.jgrid.cmTemplate[ts.p.colModel[i].template]
              : {}
            : ts.p.colModel[i].template;
        ts.p.colModel[i] = $.extend(
          true,
          {},
          ts.p.cmTemplate,
          colTemplate || {},
      ...
      if (this.p.multiselect) {
        ...
        this.p.colModel.unshift({
          ...
          width: $.jgrid.cell_width
            ? ts.p.multiselectWidth + ts.p.cellLayout
            : ts.p.multiselectWidth,
          sortable: false,
          resizable: false,
          hidedlg: true,
          search: false,
      ...
      imgs +=
        "<span sort='asc'  class='ui-grid-ico-sort ui-icon-asc" +
        iac +
        " ui-sort-" +
        dir +
        " " +
        ...
        iconbase +
        " " +
        getstyle(stylemodule, "icon_asc", true) +
        "'" +
        sort +
        "></span>";
      imgs +=
        ...
        iconbase +
        " " +
        getstyle(stylemodule, "icon_desc", true) +
        "'" +
        sort +
        "></span></span>";
      if (ts.p.multiSort) {
      ...
      for (i = 0; i < this.p.colNames.length; i++) {
        var tooltip = ts.p.headertitles
          ? ' title="' +
            (ts.p.colModel[i].tooltip
              ? ts.p.colModel[i].tooltip
        ...
        if (ts.p.colMenu && tmpcm.colmenu) {
          thead +=
            "<a class='" +
            (ts.p.direction === "ltr" ? "colmenu" : "colmenu-rtl") +
            "'><span class='colmenuspan " +
            iconbase +
            " " +
            colmenustyle.icon_menu +
            "'></span></a>";
        }
        thead += "</div></th>";
      }
      ...
      if (this.p.multiselect) {
        ...
        $("#cb_" + $.jgrid.jqID(ts.p.id), this).on("click", function() {
                ...
                if (
                  ...
                  $(
                    "#jqg_" +
                      ...
                      "_" +
                      $.jgrid.jqID(this.id)
                  )[ts.p.useProp ? "prop" : "attr"]("checked", true);
                  $(this)
                    .addClass(highlight)
                  ...
                  if (froz) {
                    ...
                    $("#" + $.jgrid.jqID(this.id), ts.grid.fbDiv).addClass(
                      highlight
                    );
                  }
                }
                ...
                if (
                  ...
                  $(
                    "#jqg_" +
                      ...
                      "_" +
                      $.jgrid.jqID(this.id)
                  )[ts.p.useProp ? "prop" : "attr"]("checked", false);
                  $(this)
                  ...
                  if (froz) {
                    ...
                    $("#" + $.jgrid.jqID(this.id), ts.grid.fbDiv).removeClass(
                      highlight
                    );
                  }
                }
          ...
          $(ts).triggerHandler("jqGridSelectAll", [
            chk ? ts.p.selarrrow : emp,
            chk
          ]);
      ...
      var tfoot = "",
        trhead = "",
        bstw =
          ts.p.styleUI.search("Bootstrap") !== -1 && !isNaN(ts.p.height)
            ? 2
            : 0;
      setColWidth();
      $(eg)
      ...
      if (ts.p.footerrow) {
        tfoot +=
          ...
          "><tbody><tr role='row' " +
          getstyle(stylemodule, "footerBox", false, "footrow footrow-" + dir) +
          ">";
      }
      if (ts.p.headerrow) {
        trhead +=
          ...
          getstyle(
            ...
            "headerRowTable",
            false,
            "ui-jqgrid-hrtable ui-common-table"
          ) +
          "><tbody><tr role='row' " +
          getstyle(
            stylemodule,
          ...
          if (sort) {
            if (ts.p.multiSort) {
              if (ts.p.viewsortcols[0]) {
                ...
                if (tmpcm.lso) {
                  $("div span.ui-icon-" + tmpcm.lso, this)
      ...
      if (ts.p.pager) {
        ...
        $(ts.p.pager)
          .css({ width: grid.width - bstw + "px" })
          .addClass(getstyle(stylemodule, "pagerBox", true, "ui-jqgrid-pager"))
          .appendTo(eg);
        if (hg) {
          $(ts.p.pager).hide();
        }
        ...
        if ((ts.p.multiselect && ts.p.multiboxonly) || ts.p.multimail) {
          if (scb) {
            ...
            $(ts.p.selarrrow).each(function(i, n) {
              ...
              $("#jqg_" + $.jgrid.jqID(ts.p.id) + "_" + $.jgrid.jqID(n))[
                ts.p.useProp ? "prop" : "attr"
              ]("checked", false);
              if (frz) {
                $("#" + $.jgrid.jqID(n), "#" + $.jgrid.jqID(frz)).removeClass(
                  highlight
                );
                $(
                  "#jqg_" + $.jgrid.jqID(ts.p.id) + "_" + $.jgrid.jqID(n),
      ...
      $(ts)
        ...
        .on({
          click: function(e) {
            ...
            if (
              $(ptr).length === 0 ||
              ptr[0].className.indexOf(disabled) > -1 ||
              (
                $(td, ts)
            ...
            var scb = $(td)
                .filter(":enabled")
                .hasClass("cbox"),
              cSel = $(ts).triggerHandler("jqGridBeforeSelectRow", [
                ptr[0].id,
                e
              ]);
            ...
            if (ts.p.multimail && ts.p.multiselect) {
              if (e.shiftKey) {
                if (scb) {
                  ...
                  if (perform_select) {
                    $.each($(this).getDataIDs(), function(_, id) {
                      if (
                        (shouldSelectRow = id === startID || shouldSelectRow)
                      ) {
                  ...
                  ts.p.selrow =
                    CurrentSelectIndex > InitialSelectIndex ? endID : startID;
                  return;
                }
              ...
              if (e[ts.p.multikey]) {
                ...
                scb = $("#jqg_" + $.jgrid.jqID(ts.p.id) + "_" + ri).is(
                  ":checked"
                );
                $("#jqg_" + $.jgrid.jqID(ts.p.id) + "_" + ri)[
                  ts.p.useProp ? "prop" : "attr"
                ]("checked", !scb);
              }
            }
          ...
          reloadGrid: function(e, opts) {
            ...
            if (opts.page) {
              ...
              if (ts.grid.prevRowHeight) {
                ts.grid.bDiv.scrollTop =
                  (page - 1) * ts.grid.prevRowHeight * ts.p.rowNum;
              } else {
          ...
          dblclick: function(e) {
            ...
            ri = ptr[0].rowIndex;
            ci = $.jgrid.getCellIndex(td);
            var dbcr = $(ts).triggerHandler("jqGridDblClickRow", [
              $(ptr).attr("id"),
              ri,
              ci,
              e
          ...
          contextmenu: function(e) {
            ...
            ri = ptr[0].rowIndex;
            ci = $.jgrid.getCellIndex(td);
            var rcr = $(ts).triggerHandler("jqGridRightClickRow", [
              $(ptr).attr("id"),
              ri,
              ci,
              e
      ...
      if (ts.p.multikey) {
        if ($.jgrid.msie()) {
          $(grid.bDiv).on("selectstart", function() {
            return false;
          });
        } else {
          $(grid.bDiv).on("mousedown", function() {
            return false;
          });
        }
      }
      ...
      var icoo =
          iconbase + " " + getstyle(stylemodule, "icon_caption_open", true),
        icoc =
          iconbase + " " + getstyle(stylemodule, "icon_caption_close", true);
        ...
        ts.p.hidegrid === true
          ? $(
              "<a role='link' class='ui-jqgrid-titlebar-close HeaderButton " +
                cornerall +
                "' title='" +
                ($.jgrid.getRegional(ts, "defaults.showhide", ts.p.showhide) ||
                  "") +
                "'" +
                " />"
            )
              .hover(
                ...
                function() {
                  arf.removeClass(hover);
                }
              )
              .append("<span class='ui-jqgrid-headlink " + icoo + "'></span>")
              .css(dir === "rtl" ? "left" : "right", "0px")
          : "";
      $(grid.cDiv)
        .append(arf)
        .append("<span class='ui-jqgrid-title'>" + ts.p.caption + "</span>")
        .addClass(
          "ui-jqgrid-titlebar ui-jqgrid-caption" +
            (dir === "rtl" ? "-rtl" : "") +
            " " +
            getstyle(stylemodule, "gridtitleBox", true)
        );
      ///// toolbar menu
      if (ts.p.menubar === true) {
        ...
        var arf1 =
          '<ul id="' +
          ts.p.id +
          '_menubar" class="ui-search-menu modal-content column-menu ui-menu jqgrid-caption-menu ' +
          colmenustyle.menu_widget +
          '" role="menubar" tabindex="0"></ul>';
        $("#gbox_" + ts.p.id).append(arf1);
        $(grid.cDiv).append(
          "<a role='link' class='ui-jqgrid-menubar menubar-" +
            (dir === "rtl" ? "rtl" : "ltr") +
            "' style=''><span class='colmenuspan " +
            iconbase +
            " " +
            colmenustyle.icon_toolbar_menu +
            "'></span></a>"
        );
        $(".ui-jqgrid-menubar", grid.cDiv)
          .hover(
            function() {
              $(this).addClass(hover);
            },
          ...
          .on("click", function(e) {
            ...
            if (ts.p.direction === "rtl") {
              $("#" + ts.p.id + "_menubar").css({
                left: pos.left - $("#" + ts.p.id + "_menubar").width() - 20
              });
            }
          });
      }
      ...
      if (ts.p.toolbar[0]) {
        ...
        if (ts.p.toolbar[1] === "both") {
          ...
          $(grid.uDiv)
            .addClass(tbstyle + " ui-userdata-top")
            .attr("id", "t_" + this.id)
            .insertBefore(grid.hDiv)
            .width(grid.width - bstw);
          $(grid.ubDiv)
            .addClass(tbstyle + " ui-userdata-bottom")
            .attr("id", "tb_" + this.id)
            .insertAfter(grid.hDiv)
            .width(grid.width - bstw);
          if (hg) {
            $(grid.ubDiv).hide();
          }
      ...
      if (ts.p.toppager) {
        ...
        $(grid.topDiv)
          .addClass(
            getstyle(stylemodule, "toppagerBox", true, "ui-jqgrid-toppager")
          )
          .width(grid.width - bstw)
          .insertBefore(grid.hDiv);
      ...
      if (ts.p.footerrow) {
        ...
        if (ts.p.rownumbers) {
          grid.footers[0].className = getstyle(
            stylemodule,
            "rownumBox",
            true,
            "jqgrid-rownum"
      ...
      if (ts.p.headerrow) {
        ...
        if (ts.p.rownumbers) {
          grid.hrheaders[0].className = getstyle(
            stylemodule,
            "rownumBox",
            true,
            "jqgrid-rownum"
      ...
      if (ts.p.caption) {
        ...
        if (ts.p.hidegrid === true) {
          $(".ui-jqgrid-titlebar-close", grid.cDiv).click(function(e) {
            var onHdCl = $.isFunction(ts.p.onHeaderClick),
              elems =
                ".ui-jqgrid-bdiv, .ui-jqgrid-hdiv, .ui-jqgrid-toppager, .ui-jqgrid-pager, .ui-jqgrid-sdiv, .ui-jqgrid-hrdiv",
              counter,
              self = this;
            ...
            if (ts.p.gridstate === "visible") {
              $(elems, "#gbox_" + $.jgrid.jqID(ts.p.id)).slideUp(
                "fast",
                function() {
                  counter--;
                  if (counter === 0) {
                    ...
                    if (
                      $("#gbox_" + $.jgrid.jqID(ts.p.id)).hasClass(
                        "ui-resizable"
                      )
                    ) {
                      $(
                        ".ui-resizable-handle",
                        "#gbox_" + $.jgrid.jqID(ts.p.id)
                      ).hide();
                    }
                    $(ts).triggerHandler("jqGridHeaderClick", [
                      ts.p.gridstate,
                      e
                    ]);
                    if (onHdCl) {
              ...
              $(elems, "#gbox_" + $.jgrid.jqID(ts.p.id)).slideDown(
                "fast",
                function() {
                  counter--;
                  if (counter === 0) {
        ...
        if (!ts.p.toppager) {
          $(grid.hDiv).addClass(
            getstyle(ts.p.styleUI + ".common", "cornertop", true)
          );
        }
      }
      if (ts.p.headerrow) {
      ...
      $(".ui-jqgrid-labels", grid.hDiv).on("selectstart", function() {
        return false;
      });
      ...
      ts.addLocalData = function(d) {
        return addLocalData(d);
      };
      ts.treeGrid_beforeRequest = function() {
        treeGrid_beforeRequest();
      }; //bvn13
      ts.treeGrid_afterLoadComplete = function() {
        treeGrid_afterLoadComplete();
      };
  ...
  $.jgrid.extend({
    ...
    setSelection: function(selection, onsr, e, isHight) {
      return this.each(function() {
        ...
        if (!pt || !pt.className || pt.className.indexOf(disabled) > -1) {
          return;
        }
        function scrGrid(iR) {
        ...
        if (!$t.p.multiselect) {
          if (pt.className !== "ui-subgrid") {
            if ($t.p.selrow !== pt.id) {
              if (isHight) {
                ...
                $(pt)
                  .addClass(highlight)
                  .attr({ "aria-selected": "true", tabindex: "0" }); //.focus();
                if (fid) {
                  $(
          ...
          $("#jqg_" + $.jgrid.jqID($t.p.id) + "_" + $.jgrid.jqID(pt.id))[
          ...
          if (fid) {
            ...
            $(
              "#jqg_" + $.jgrid.jqID($t.p.id) + "_" + $.jgrid.jqID(selection),
              "#" + $.jgrid.jqID(fid)
            )[$t.p.useProp ? "prop" : "attr"]("checked", stat);
          }
          if (onsr) {
            $($t).triggerHandler("jqGridSelectRow", [pt.id, stat, e]);
            if ($t.p.onSelectRow) {
      ...
      return this.each(function() {
        ...
        if (rowid !== undefined) {
          ...
          $("#" + $.jgrid.jqID(t.p.id) + " tbody:first tr#" + $.jgrid.jqID(sr))
            ...
            .attr("aria-selected", "false");
          if (fid) {
            $("#" + $.jgrid.jqID(sr), "#" + $.jgrid.jqID(fid)).removeClass(
              highlight
            );
          }
          if (t.p.multiselect) {
            $(
              "#jqg_" + $.jgrid.jqID(t.p.id) + "_" + $.jgrid.jqID(sr),
              "#" + $.jgrid.jqID(t.p.id)
          ...
          if (t.p.onUnSelectRow) {
            t.p.onUnSelectRow.call(t, sr);
          }
          sr = null;
          ...
          if (t.p.selrow) {
            ...
            if (t.p.onUnSelectRow) {
              t.p.onUnSelectRow.call(t, t.p.selrow);
            }
            t.p.selrow = null;
          }
        } else {
          $(t.p.selarrrow).each(function(i, n) {
            $($(t).jqGrid("getGridRowById", n))
              .removeClass(highlight)
              .attr("aria-selected", "false");
            $("#jqg_" + $.jgrid.jqID(t.p.id) + "_" + $.jgrid.jqID(n))[
              t.p.useProp ? "prop" : "attr"
            ]("checked", false);
            if (fid) {
              $("#" + $.jgrid.jqID(n), "#" + $.jgrid.jqID(fid)).removeClass(
                highlight
              );
              $(
                "#jqg_" + $.jgrid.jqID(t.p.id) + "_" + $.jgrid.jqID(n),
      ...
      this.each(function() {
        ...
        while (j < len) {
          ...
          if ($(ind).hasClass("jqgrow")) {
            // ignore first not visible row
            if (usedata) {
              res =
                $t.p.data[
                  $t.p._index[$.jgrid.stripPref($t.p.idPrefix, ind.id)]
                ];
            } else {
              $('td[role="gridcell"]', ind).each(function(i) {
                ...
                if (nm !== "cb" && nm !== "subgrid" && nm !== "rn") {
                  if ($t.p.treeGrid === true && nm === $t.p.ExpandColumn) {
                    ...
                    try {
                      res[nm] = $.unformat.call(
                        $t,
                        this,
                        { rowId: ind.id, colModel: $t.p.colModel[i] },
                        i
                      );
                    } catch (e) {
            ...
            $(this.p.colModel).each(function(i) {
              ...
              if (dval !== undefined) {
                lcdata[nm] =
                  ...
                  typeof this.formatter === "string" &&
                  this.formatter === "date"
                    ? $.unformat.date.call(t, dval, this)
                    : dval;
                vl = t.formatter(rowid, lcdata[nm], i, data, "edit");
                title = this.title ? { title: $.jgrid.stripHtml(vl) } : {};
                if (t.p.treeGrid === true && nm === t.p.ExpandColumn) {
                  $("td[role='gridcell']:eq(" + i + ") > span:first", ind)
                    .html(vl)
                    .attr(title);
                } else {
                  $("td[role='gridcell']:eq(" + i + ")", ind)
                    .html(vl)
                    .attr(title);
                }
              }
        ...
        $(this.p.colModel).each(function(i) {
          ...
          if (action === "set") {
            if (data[nm] !== undefined) {
              vl = format
                ? t.formatter("", data[nm], i, data, "edit")
                : data[nm];
              title = this.title ? { title: $.jgrid.stripHtml(vl) } : {};
              $("tr.footrow td:eq(" + i + ")", t.grid.sDiv)
                .html(vl)
                .attr(title);
              success = true;
            }
          ...
          if (action === "set") {
            if (data[nm] !== undefined) {
              vl = format
                ...
                : data[nm];
              title = this.title ? { title: $.jgrid.stripHtml(vl) } : {};
              $("tr.hrheadrow td:eq(" + i + ")", t.grid.hrDiv)
                .html(vl)
                .attr(title);
              success = true;
            }
    ...
    showHideCol: function(colname, show) {
      return this.each(function() {
        ...
        var sw = show === "" ? true : false,
          gh =
            $t.p.groupHeader &&
            ($.isArray($t.p.groupHeader) || $.isFunction($t.p.groupHeader));
        if (gh) {
        ...
        $(this.p.colModel).each(function(i) {
          if ($.inArray(this.name, colname) !== -1 && this.hidden === sw) {
            ...
            if ($t.p.headerrow) {
              $("tr.hrheadrow td:eq(" + i + ")", $t.grid.hrDiv).css(
                "display",
                show
              );
            }
        ...
        if (fndh === true) {
          ...
          $($t).jqGrid(
            "setGridWidth",
            $t.p.shrinkToFit === true ? $t.p.tblwidth : $t.p.width
          );
        }
        if (gh) {
    ...
    remapColumns: function(permutation, updateCells, keepHeader) {
      ...
      resortRows(
        $("thead:first", ts.grid.hDiv),
        keepHeader && ":not(.ui-jqgrid-labels)"
      );
      if (updateCells) {
        resortRows(
    ...
    setGridWidth: function(nwidth, shrink) {
      return this.each(function() {
        ...
        var $t = this,
          ...
          cr,
          bstw =
            $t.p.styleUI.search("Bootstrap") !== -1 && !isNaN($t.p.height)
              ? 2
              : 0;
        ...
        if ($t.p.toolbar[0] === true) {
          ...
          if ($t.p.toolbar[1] === "both") {
            $($t.grid.ubDiv).css("width", nwidth - bstw + "px");
          }
        }
        if ($t.p.footerrow) {
        ...
        if (shrink === true) {
          ...
          if (!isNaN($t.p.height)) {
            if (
              $($t.grid.bDiv)[0].clientHeight <
                $($t.grid.bDiv)[0].scrollHeight ||
              $t.rows.length === 1 ||
              $($t.grid.bDiv).css("overflow-y") === "scroll"
            ) {
              hs = true;
              aw -= scw;
            }
          ...
          if (hs) {
            ...
            !hs &&
            Math.abs(nwidth - gw - (initwidth + brd * vc)) !== 0
          ) {
    ...
    setCaption: function(newcap) {
      return this.each(function() {
        var ctop = $(this).jqGrid(
          "getStyleUI",
          this.p.styleUI + ".common",
          "cornertop",
          true
        );
        this.p.caption = newcap;
        $(".ui-jqgrid-title, .ui-jqgrid-title-rtl", this.grid.cDiv).html(
          newcap
        );
        $(this.grid.cDiv).show();
                ...
                if ($t.p.datatype === "local") {
                  ...
                  while (cl < ind.cells.length) {
                    ...
                    v = $.unformat.call(
                      $t,
                      $(ind.cells[cl]),
                      { rowId: ind.id, colModel: $t.p.colModel[cl] },
                      cl
                    );
                    rawdat.push(v);
                    cl++;
            ...
            if (returnobject) {
              ...
              try {
                ret = $.unformat.call(
                  $t,
                  obj,
                  { rowId: ind.id, colModel: $t.p.colModel[pos] },
                  pos
                );
              } catch (e) {
            ...
            for (; i < ln; i++) {
              if ($($t.rows[i]).hasClass("jqgrow")) {
                if (mathopr === "maxwidth") {
                  ...
                  max = Math.max(
                    $.jgrid.getTextWidth($t.rows[i].cells[pos].innerHTML, font),
                    max
                  );
                  continue;
                }
                ...
                try {
                  val = $.unformat.call(
                    $t,
                    $($t.rows[i].cells[pos]),
                    { rowId: $t.rows[i].id, colModel: $t.p.colModel[pos] },
                    pos
                  );
                } catch (e) {
    ...
    clearGridData: function(clearfooter, clearheader) {
      return this.each(function() {
        ...
        if ($t.p.headerrow && clearheader) {
          $(".ui-jqgrid-hrtable td", $t.grid.hrDiv).html("&#160;");
        }
        $t.p.selrow = null;
        $t.p.selarrrow = [];
        $t.p.savedRow = [];
    ...
    sortGrid: function(colname, reload, sor) {
      return this.each(function() {
        ...
        for (i = 0; i < $t.p.colModel.length; i++) {
          if (
            $t.p.colModel[i].index === colname ||
            $t.p.colModel[i].name === colname
          ) {
            idx = i;
            if (
              $t.p.frozenColumns === true &&
              $t.p.colModel[i].frozen === true
            ) {
    ...
    setGridState: function(state) {
      return this.each(function() {
        ...
        if (state === "hidden") {
          ...
          if ($t.p.footerrow) {
            $(".ui-jqgrid-sdiv", "#gbox_" + $.jgrid.jqID($t.p.id)).slideUp(
              "fast"
            );
          }
          if ($t.p.headerrow) {
            $(".ui-jqgrid-hrdiv", "#gbox_" + $.jgrid.jqID($t.p.id)).slideUp(
              "fast"
            );
          }
          $(".ui-jqgrid-headlink", $t.grid.cDiv)
            .removeClass(open)
            .addClass(close);
          ...
          if ($t.p.footerrow) {
            $(".ui-jqgrid-sdiv", "#gbox_" + $.jgrid.jqID($t.p.id)).slideDown(
              "fast"
            );
          }
          if ($t.p.headerrow) {
            $(".ui-jqgrid-hrdiv", "#gbox_" + $.jgrid.jqID($t.p.id)).slideDown(
              "fast"
            );
          }
          $(".ui-jqgrid-headlink", $t.grid.cDiv)
    ...
    setFrozenColumns: function() {
      return this.each(function() {
        ...
        var $t = this,
          ...
          borderbox =
            $("#gbox_" + $.jgrid.jqID($t.p.id)).css("box-sizing") ===
            "border-box",
        ...
        if (maxfrozen >= 0 && frozen) {
          var top = $t.p.caption ? $($t.grid.cDiv).outerHeight() : 0,
            ...
            divhth = parseInt(
              $(".ui-jqgrid-hdiv", "#gview_" + $.jgrid.jqID($t.p.id)).height(),
              10
            );
          //headers
          if ($t.p.toppager) {
          ...
          if ($t.p.headerrow) {
            hhrh = parseInt(
              $(".ui-jqgrid-hrdiv", "#gview_" + $.jgrid.jqID($t.p.id)).height(),
              10
            );
          }
          $t.grid.fhDiv = $(
            '<div style="position:absolute;' +
              ($t.p.direction === "rtl" ? "right:0;" : "left:0;") +
              "top:" +
              top +
              "px;height:" +
              (divhth - pixelfix) +
          ...
          if ($t.p.groupHeader) {
            $("tr.jqg-first-row-header, tr.jqg-third-row-header", htbl).each(
              function() {
            ...
            var maxdh = [];
            $(".ui-jqgrid-htable tr", "#gview_" + $.jgrid.jqID($t.p.id)).each(
              function(i, n) {
          ...
          if ($t.p.headerrow) {
            $t.grid.fhrDiv = $(
              '<div style="position:absolute;' +
                ($t.p.direction === "rtl" ? "right:0;" : "left:0;") +
                "top:" +
                (parseInt(top, 10) + parseInt(divhth, 10) + 1 - pixelfix) +
                "px;height:" +
                (divhth - pixelfix) +
                'px;" class="frozen-hrdiv ui-jqgrid-hrdiv "></div>'
            );
          ...
          $($t).on("jqGridAfterGridComplete.setFrozenColumns", function() {
            ...
            hasscroll =
              parseInt($($t.grid.bDiv)[0].scrollWidth, 10) >
              parseInt($($t.grid.bDiv)[0].clientWidth, 10);
            $($t.grid.fbDiv).height(
              $($t.grid.bDiv)[0].clientHeight -
                (hasscroll ? 0 : $t.p.scrollOffset - 3)
            );
            // find max height
            var mh = [];
            $("#" + $.jgrid.jqID($t.p.id) + " tr[role=row].jqgrow").each(
              function() {
    ...
    resizeColumn: function(iCol, newWidth, forceresize) {
      return this.each(function() {
        ...
        if (p.forceFit) {
          ...
          if (diffnv < p.minColWidth) {
            return;
          }
          grid.headers[iCol + p.nv].newWidth =
            grid.headers[iCol + p.nv].width - diff;
        }
        ...
        switch (stylemod.length) {
          case 1:
            ret +=
              q +
              $.trim(
                gridclass + " " + $.jgrid.styleUI[stylemod[0]][classui] + q
              );
            break;
          case 2:
            ret +=
              ...
              $.trim(
                gridclass +
                  " " +
                  $.jgrid.styleUI[stylemod[0]][stylemod[1]][classui] +
                  q
              );
        }
      } catch (cls) {
        ...
        setTimeout(function() {
          try {
            var winwidth = $(window).width(),
              parentwidth = $("#gbox_" + $.jgrid.jqID($t.p.id))
                .parent()
                .width(),
              ww = $t.p.width;
    ...
    menubarAdd: function(items) {
      var currstyle = this[0].p.styleUI,
        styles = $.jgrid.styleUI[currstyle].common,
        item,
        str;
        ...
        if ($.isArray(items)) {
          for (var i = 0; i < items.length; i++) {
            ...
            if (item.divider) {
              ...
              str =
                '<li class="ui-menu-item" role="presentation"><a id="' +
                item.id +
    ...
    editCell: function(iRow, iCol, ed, event) {
      return this.each(function() {
        var $t = this,
          ...
          inpclass = $(this).jqGrid(
            "getStyleUI",
            $t.p.styleUI + ".celledit",
            "inputClass",
            true
          );
        ...
        if (
          ...
          (!$.isFunction($t.p.isCellEditable) ||
            $t.p.isCellEditable.call($t, nm, iRow, iCol))
        ) {
          try {
            tmp = $.unformat.call(
              $t,
              cc,
              { rowId: $t.rows[iRow].id, colModel: cm },
              iCol
            );
          } catch (_) {
            tmp =
              cm.edittype && cm.edittype === "textarea" ? cc.text() : cc.html();
          }
          if ($t.p.autoencode) {
          ...
          if (
            tmp === "&nbsp;" ||
            tmp === "&#160;" ||
            (tmp.length === 1 && tmp.charCodeAt(0) === 160)
          ) {
            tmp = "";
          }
          if ($.isFunction($t.p.formatCell)) {
            var tmp2 = $t.p.formatCell.call(
              $t,
              $t.rows[iRow].id,
              nm,
              tmp,
          ...
          $($t).triggerHandler("jqGridBeforeEditCell", [
            $t.rows[iRow].id,
            nm,
            tmp,
            iRow,
          ...
          if (
            $.inArray(cm.edittype, ["text", "textarea", "password", "select"]) >
            -1
          ) {
          ...
          $($t).triggerHandler("jqGridAfterEditCell", [
            $t.rows[iRow].id,
            nm,
            tmp,
            iRow,
          ...
          $($t).triggerHandler("jqGridCellSelect", [
            $t.rows[iRow].id,
            iCol,
            tmp,
            event
      ...
      return this.each(function() {
        ...
        if (fr !== null) {
          ...
          switch (cm.edittype) {
            case "select":
              if (!cm.editoptions.multiple) {
                ...
                v2 = $(
                  "#" + iRow + "_" + nmjq + " option:selected",
                  trow
                ).text();
              } else {
                var sel = $("#" + iRow + "_" + nmjq, trow),
                  selectedText = [];
                v = $(sel).val();
                if (v) {
                  v.join(",");
            ...
            case "custom":
              try {
                if (
                  ...
                  v = cm.editoptions.custom_value.call(
                    $t,
                    $(".customelement", cc),
                    "get"
                  );
                ...
                if (e === "e1") {
                  $.jgrid.info_dialog(
                    errors.errcap,
                    "function 'custom_value' " + edit.msg.nodefined,
                    edit.bClose,
                    { styleUI: $t.p.styleUI }
                  );
                } else if (e === "e2") {
                  $.jgrid.info_dialog(
                    errors.errcap,
                    "function 'custom_value' " + edit.msg.novalue,
                    edit.bClose,
                    { styleUI: $t.p.styleUI }
                  );
                } else {
          ...
          if (v2 !== $t.p.savedRow[fr].v) {
            var vvv = $($t).triggerHandler("jqGridBeforeSaveCell", [
              $t.p.savedRow[fr].rowId,
              nm,
              v,
              iRow,
              iCol
            ...
            if (cv[0] === true) {
              var addpost =
                $($t).triggerHandler("jqGridBeforeSubmitCell", [
                  $t.p.savedRow[fr].rowId,
                  nm,
                  v,
                  iRow,
              ...
              if ($.isFunction($t.p.beforeSubmitCell)) {
                addpost = $t.p.beforeSubmitCell.call(
                  $t,
                  $t.p.savedRow[fr].rowId,
                  nm,
                  v,
              ...
              var retsub = $($t).triggerHandler("jqGridOnSubmitCell", [
                $t.p.savedRow[fr].rowId,
                nm,
                v,
                iRow,
              ...
              if ($t.p.cellsubmit === "remote") {
                if ($t.p.cellurl) {
                  ...
                  postdata[idname] = $.jgrid.stripPref(
                    $t.p.idPrefix,
                    $t.p.savedRow[fr].rowId
                  );
                  ...
                  $.ajax(
                    $.extend(
                      {
                        url: $t.p.cellurl,
                        data: $.isFunction($t.p.serializeCellData)
                          ? $t.p.serializeCellData.call($t, postdata, nm)
                          : postdata,
                        type: "POST",
                        complete: function(result, stat) {
                          ...
                          if (stat === "success") {
                            var ret = $($t).triggerHandler(
                              "jqGridAfterSubmitCell",
                              [$t, result, postdata[idname], nm, v, iRow, iCol]
                            ) || [true, ""];
                            if (
                              ret[0] === true &&
                              $.isFunction($t.p.afterSubmitCell)
                            ) {
                              ret = $t.p.afterSubmitCell.call(
                                $t,
                                result,
                                postdata[idname],
                                nm,
                            ...
                            if (ret[0] === true) {
                              ...
                              $($t).triggerHandler("jqGridAfterSaveCell", [
                                $t.p.savedRow[fr].rowId,
                                nm,
                                v,
                                iRow,
                              ...
                              $($t).triggerHandler("jqGridErrorCell", [
                                result,
                                stat
                              ]);
    ...
    restoreCell: function(iRow, iCol) {
      return this.each(function() {
        ...
        if (fr !== null) {
          ...
          $($t).triggerHandler("jqGridAfterRestoreCell", [
            $t.p.savedRow[fr].rowId,
            $t.p.savedRow[fr].v,
            iRow,
            iCol
    ...
    nextCell: function(iRow, iCol, event) {
      ...
      this.each(function() {
        ...
        for (i = iCol + 1; i < $t.p.colModel.length; i++) {
          if (
            $t.p.colModel[i].editable === true &&
            (!$.isFunction($t.p.isCellEditable) ||
              $t.p.isCellEditable.call($t, $t.p.colModel[i].name, iRow, i))
          ) {
            nCol = i;
            break;
          }
      ...
      return this.each(function() {
        ...
        function scrollGrid(iR, iC, tp) {
          if (tp.substr(0, 1) === "v") {
            ...
            if (tp === "vd") {
              if (nROT >= ch) {
                $($t.grid.bDiv)[0].scrollTop =
                  $($t.grid.bDiv)[0].scrollTop + $t.rows[iR].clientHeight;
              }
            }
            if (tp === "vu") {
              if (pROT < st) {
          ...
          if (tp === "h") {
            var cw = $($t.grid.bDiv)[0].clientWidth,
              ...
              nCOL =
                $t.rows[iR].cells[iC].offsetLeft +
                $t.rows[iR].cells[iC].clientWidth,
              pCOL = $t.rows[iR].cells[iC].offsetLeft;
            if (nCOL >= cw + parseInt(sl, 10)) {
              $($t.grid.bDiv)[0].scrollLeft =
                $($t.grid.bDiv)[0].scrollLeft +
                $t.rows[iR].cells[iC].clientWidth;
            } else if (pCOL < sl) {
              $($t.grid.bDiv)[0].scrollLeft =
                  ...
                  if ($(this).hasClass("dirty-cell")) {
                    try {
                      res[nm] = $.unformat.call(
                        $t,
                        this,
                        { rowId: $t.rows[j].id, colModel: $t.p.colModel[i] },
                        i
                      );
                    } catch (e) {
                      res[nm] = $.jgrid.htmlDecode($(this).html());
  ...
  $.extend($.jgrid, {
    ...
    hideModal: function(selector, o) {
      o = $.extend(
        { jqm: true, gb: "", removemodal: false, formprop: false, form: "" },
        o || {}
      );
      var thisgrid =
        o.gb && typeof o.gb === "string" && o.gb.substr(0, 6) === "#gbox_"
          ? $("#" + o.gb.substr(6))[0]
          : false;
      if (o.onClose) {
        var oncret = thisgrid
          ? o.onClose.call(thisgrid, selector)
          : o.onClose(selector);
    ...
    createModal: function(
      ...
      var ahr = $(
        "<a class='ui-jqdialog-titlebar-close " + common.cornerall + "'></a>"
      )
        .hover(
          function() {
            ahr.addClass(common.hover);
          },
          function() {
            ahr.removeClass(common.hover);
          }
        )
        .append(
          "<span class='" +
      ...
      $(mc)
        .addClass("ui-jqdialog-content " + classes.content)
      ...
      $("a.ui-jqdialog-titlebar-close", mh).click(function() {
        var oncm =
          $("#" + $.jgrid.jqID(aIDs.themodal)).data("onClose") || p.onClose;
        var gboxclose =
          $("#" + $.jgrid.jqID(aIDs.themodal)).data("gbox") || p.gbox;
      ...
      if (rtlsup && coord.left && !appendsel) {
        rtlt =
          $(p.gbox).width() - (!isNaN(p.width) ? parseInt(p.width, 10) : 0) - 8; // to do
        // just in case
        ...
        if ($.fn.jqResize) {
          $(mw).append(
            "<div class='jqResize " +
              ...
              common.icon_base +
              " " +
              classes.icon_resizable +
              "'></div>"
          );
          $("#" + $.jgrid.jqID(aIDs.themodal)).jqResize(
            ".jqResize",
            aIDs.scrollelm ? "#" + $.jgrid.jqID(aIDs.scrollelm) : false
          );
        } else {
          try {
            $(mw).resizable({
    ...
    viewModal: function(selector, o) {
      ...
      if (o.gbox) {
        ...
        try {
          style = $(grid).jqGrid(
            "getStyleUI",
            grid.p.styleUI + ".common",
            "overlay",
            false,
            "jqgrid-overlay-modal"
          );
          o.overlayClass = $(grid).jqGrid(
            "getStyleUI",
            grid.p.styleUI + ".common",
            "overlay",
            true
      ...
      if ($.fn.jqm && o.jqm === true) {
        ...
        if (o.focusField >= 0) {
          try {
            $(":input:visible", selector)[o.focusField].focus();
          } catch (_) {}
        }
      }
    },
    info_dialog: function(caption, content, c_b, modalopt) {
      ...
      cnt += c_b
        ? "<div class='" +
          ...
          "' style='text-align:" +
          mopt.buttonalign +
          ";padding-bottom:0.8em;padding-top:0.5em;background-image: none;border-width: 1px 0 0 0;'><a id='closedialog' class='fm-button " +
          common.button +
          "'>" +
          c_b +
          "</a>" +
      ...
      switch (eltype) {
        ...
        case "checkbox": //what code for simple checkbox
          ...
          if (!options.value) {
            ...
            if (
              vl1.search(/(false|f|0|no|n|off|undefined)/i) < 0 &&
              vl1 !== ""
            ) {
              elem.checked = true;
          ...
          if (options.dataUrl != null) {
              ...
              $.extend(
                {
                  ...
                  type: "GET",
                  dataType: "html",
                  data: $.isFunction(postData)
                    ? postData.call($t, rowid, vl, String(options.name))
                    : postData,
                  ...
                  success: function(data) {
                    var ovm = [],
                      ...
                      txt,
                      mss = [],
                      a = $.isFunction(options.buildSelect)
                        ? options.buildSelect.call($t, data)
                        : data;
                    if (typeof a === "string") {
                      a = $($.trim(a)).html();
                    }
                    if (a) {
                      ...
                      $("option", elem).each(function(i) {
                        ...
                        if (
                          $.inArray($.trim(txt), ovm) > -1 ||
                          $.inArray($.trim(vl), ovm) > -1
                        ) {
                      ...
                      if (cU) {
                        if (options.oper === "edit") {
                          ...
                          $($t).jqGrid("setColProp", options.name, {
                            searchoptions: { dataUrl: null, value: oV }
                          });
            ...
            if (typeof options.value === "string") {
              ...
              for (i = 0; i < so.length; i++) {
                ...
                if (
                  !msl &&
                  ($.trim(sv[0]) === $.trim(vl) || $.trim(sv[1]) === $.trim(vl))
                ) {
              ...
              for (i = 0; i < oSv.length; i++) {
                if (oSv[i].length === 2) {
                  ...
                  if (
                    !msl &&
                    ($.trim(key) === $.trim(vl) || $.trim(value) === $.trim(vl))
                  ) {
                    ov.selected = "selected";
                  }
                  if (
                    msl &&
                    ($.inArray($.trim(value), ovm) > -1 ||
                      $.inArray($.trim(key), ovm) > -1)
                  ) {
                    ov.selected = "selected";
                  }
                }
              ...
              for (key in oSv) {
                if (oSv.hasOwnProperty(key)) {
                  ...
                  if (
                    !msl &&
                    ($.trim(key) === $.trim(vl) ||
                      $.trim(oSv[key]) === $.trim(vl))
                  ) {
                    ov.selected = "selected";
                  }
                  if (
                    msl &&
                    ($.inArray($.trim(oSv[key]), ovm) > -1 ||
                      $.inArray($.trim(key), ovm) > -1)
                  ) {
                    ov.selected = "selected";
                  }
        ...
        case "custom":
          ...
          try {
            if ($.isFunction(options.custom_element)) {
              ...
              if (celm) {
                celm = $(celm)
                  ...
                  .attr({ id: options.id, name: options.name });
                $(elem)
                  .empty()
                  .append(celm);
              } else {
            ...
            if (e === "e1") {
              $.jgrid.info_dialog(
                errors.errcap,
                "function 'custom_element' " + edit.msg.nodefined,
                edit.bClose,
                { styleUI: $t.p.styleUI }
              );
            } else if (e === "e2") {
              $.jgrid.info_dialog(
                errors.errcap,
                "function 'custom_element' " + edit.msg.novalue,
                edit.bClose,
                { styleUI: $t.p.styleUI }
              );
            } else {
    ...
    checkDate: function(format, date) {
      ...
      if (
        ...
        tsp[format[dln]] > 31 ||
        (tsp[format[mln]] === 2 &&
          tsp[format[dln]] > daysInFebruary(tsp[format[j]])) ||
    ...
    checkValues: function(val, valref, customobject, nam) {
      var edtrul,
        ...
        g = this,
        cm = g.p.colModel,
        msg = $.jgrid.getRegional(this, "edit.msg"),
        fmtdate,
        isNum = function(vn) {
          var vn = vn.toString();
          if (vn.length >= 2) {
      ...
      if (edtrul) {
        ...
        if (edtrul.date === true) {
          if (!(rqfield === false && $.jgrid.isEmpty(val))) {
            if (
              cm[valref].formatoptions &&
              cm[valref].formatoptions.newformat
            ) {
        ...
        if (typeof field.willValidate !== "undefined") {
          ...
          if (
            ...
            field.setCustomValidity(
              $.jgrid.LegacyValidation(field) ? "" : "error"
            );
          }
  ...
  $.fn.jqFilter = function(arg) {
      ...
      if (this.p.showQuery) {
        $(this).append(
          "<table class='queryresult " +
            classes.table_widget +
            "' style='display:block;max-width:440px;border:0px none;' dir='" +
            this.p.direction +
    ...
    */
      ...
      this.onchange = function() {
        ...
        this.p.error = false;
        this.p.errmsg = "";
        return $.isFunction(this.p.onChange)
      ...
      this.createTableForGroup = function(group, parentgroup) {
        ...
        var table = $(
            "<table class='group " +
              classes.table_widget +
              " ui-search-table' style='border:0px none;'><tbody></tbody></table>"
          ),
        ...
        if (parentgroup === null) {
          table.append(
            "<tr class='error' style='display:none;'><th colspan='5' class='" +
              common.error +
              "' align='" +
              align +
              "'></th></tr>"
        ...
        if (this.p.ruleButtons === true) {
          ...
          var groupOpSelect = $(
            "<select size='1' class='opsel " + classes.srSelect + "'></select>"
          );
          ...
          for (i = 0; i < p.groupOps.length; i++) {
            selected =
              group.groupOp === that.p.groupOps[i].op
                ? " selected='selected'"
                : "";
            str +=
              "<option value='" +
              ...
              selected +
              ">" +
              that.p.groupOps[i].text +
              "</option>";
          }

          groupOpSelect.append(str).on("change", function() {
        ...
        if (this.p.ruleButtons === true) {
          ...
          inputAddRule.on("click", function() {
            ...
            for (i = 0; i < that.p.columns.length; i++) {
              ...
              var searchable =
                  that.p.columns[i].search === undefined
                    ? true
                    : that.p.columns[i].search,
                hidden = that.p.columns[i].hidden === true,
                ignoreHiding =
                  that.p.columns[i].searchoptions.searchhidden === true;
        ...
        if (group.groups !== undefined) {
          for (i = 0; i < group.groups.length; i++) {
            ...
            tdMainHolderForSubgroup.append(
              this.createTableForGroup(group.groups[i], group)
            );
      ...
      this.createTableRowForRule = function(rule, group) {
        ...
        var ruleFieldSelect = $(
            "<select size='1' class='" + classes.srSelect + "'></select>"
          ),
          ina,
          aoprs = [];
        ...
        ruleFieldSelect.on("change", function() {
          ...
          if ($.jgrid.msie() && $.jgrid.msiever() < 9) {
            var sw =
              parseInt($("select.selectopts", trpar)[0].offsetWidth, 10) + 1;
          ...
          $(".input-elm", trpar).on("change", function(e) {
            ...
            if (
              cm.inputtype === "custom" &&
              $.isFunction(cm.searchoptions.custom_value)
            ) {
              rule.data = cm.searchoptions.custom_value.call(
                $t,
                $(".customelement", this),
                "get"
              );
        ...
        cm.searchoptions.name = rule.field;
        cm.searchoptions.oper = "filter";
        var ruleDataInput = $.jgrid.createEl.call(
          $t,
          cm.inputtype,
          cm.searchoptions,
          rule.data,
        ...
        var ruleOperatorSelect = $(
          "<select size='1' class='selectopts " +
            classes.srSelect +
            "'></select>"
        );
        ...
        ruleOperatorSelect.on("change", function() {
          ...
          if (rule.op === "nu" || rule.op === "nn") {
            // disable for operator "is null" and "is not null"
            rule.data = "";
        ...
        for (i = 0; i < op.length; i++) {
          ...
          if (ina !== -1) {
            selected =
              rule.op === that.p.ops[ina].oper ? " selected='selected'" : "";
            str +=
              "<option value='" +
              that.p.ops[ina].oper +
              "'" +
        ...
        $(ruleDataInput)
          ...
          .on("change", function() {
            rule.data =
              cm.inputtype === "custom"
                ? cm.searchoptions.custom_value.call(
                    $t,
                    $(".customelement", this),
                    "get"
                  )
      ...
      this.getStringForRule = function(rule) {
        ...
        for (i = 0; i < this.p.ops.length; i++) {
          if (this.p.ops[i].oper === rule.op) {
            opUF = this.p.operands.hasOwnProperty(rule.op)
              ? this.p.operands[rule.op]
              : "";
        ...
        if (
          $.inArray(cm.searchtype, numtypes) !== -1 ||
          opC === "nn" ||
          opC === "nu"
        ) {
  ...
  $.extend($.jgrid, {
    filterRefactor: function(p) {
      ...
      try {
        filters =
          typeof p.ruleGroup === "string"
            ? $.jgrid.parse(p.ruleGroup)
            : p.ruleGroup;
  ...
  $.jgrid.extend({
      ...
      return this.each(function() {
        ...
        if ($t.p.customFilterDef !== undefined) {
          for (var uskey in $t.p.customFilterDef) {
            if (
              $t.p.customFilterDef.hasOwnProperty(uskey) &&
              !p.operands.hasOwnProperty(uskey)
            ) {
              p.odata.push({
                oper: uskey,
                text: $t.p.customFilterDef[uskey].text
              });
        ...
        var classes = $.jgrid.styleUI[$t.p.styleUI || "jQueryUI"].filter,
          ...
          triggerToolbar = function() {
            ...
            if (
              ...
              $.each(["searchField", "searchString", "searchOper"], function(
                i,
                n
              ) {
          ...
          buildRuleMenu = function(elem, left, top) {
            ...
            for (i = 0; i < options.sopt.length; i++) {
              ...
              if (ina !== -1) {
                selclass =
                  selected === p.odata[ina].oper ? common.highlight : "";
                str +=
            ...
            $("#sopt_menu > li > a")
              ...
              .click(function() {
                var v = $(this).attr("value"),
                  oper = $(this).attr("oper");
                $($t).triggerHandler("jqGridToolbarSelectOper", [
                  v,
                  oper,
                  elem
                ]);
        ...
        function setrules(filter) {
          if (filter && filter.rules) {
            ...
            for (i = 0; i < l; i++) {
              ...
              if (ia !== -1) {
                searchitem = $(
                  "#gs_" + $t.p.idPrefix + $.jgrid.jqID(cm[ia].name)
                );
    ...
    searchGrid: function(p) {
      ...
      return this.each(function() {
        ...
        var fid = "fbox_" + $t.p.id,
          ...
          defaultFilters =
            $.isPlainObject($t.p_savedFilter) &&
            !$.isEmptyObject($t.p_savedFilter)
              ? $t.p_savedFilter
              : $t.p.postData[p.sFilter],
          fl,
        ...
        if ($("#" + $.jgrid.jqID(IDs.themodal))[0] !== undefined) {
          ...
          var columns = $.extend([], $t.p.colModel),
            bS =
              "<a id='" +
              fid +
              "_search' class='fm-button " +
              common.button +
              " fm-button-icon-right ui-search'><span class='" +
              common.icon_base +
              " " +
              classes.icon_search +
              "'></span>" +
            ...
            bC =
              "<a id='" +
              fid +
              "_reset' class='fm-button " +
              common.button +
              " fm-button-icon-left ui-reset'><span class='" +
              common.icon_base +
              " " +
              classes.icon_reset +
              "'></span>" +
          ...
          if (p.showQuery) {
            bQ =
              "<a id='" +
              fid +
              "_query' class='fm-button " +
              common.button +
              " fm-button-icon-left'><span class='" +
              common.icon_base +
              " " +
              classes.icon_query +
              "'></span>Query</a>";
            ...
            $.each(columns, function(i, n) {
              ...
              if (!found) {
                var searchable = n.search === undefined ? true : n.search,
                  ...
                  ignoreHiding =
                    n.searchoptions && n.searchoptions.searchhidden === true;
                if ((ignoreHiding && searchable) || (searchable && !hidden)) {
                  found = true;
          ...
          if ((!defaultFilters && colnm) || p.multipleSearch === false) {
            ...
            defaultFilters = {
              groupOp: "AND",
              rules: [{ field: colnm, op: cmop, data: "" }]
            };
          }
          found = false;
          if (p.tmplNames && p.tmplNames.length) {
            ...
            tmpl +=
              "<td><select size='1' class='ui-template " +
              classes.srSelect +
              "'>";
          ...
          bt =
            ...
            fid +
            "_2'><tbody><tr><td colspan='2'><hr class='" +
            common.content +
            "' style='margin:1px'/></td></tr>" +
            tmpl +
            "<tr><td class='EditButton' style='text-align:" +
            align +
            "'>" +
            bC +
          ...
          if (found && p.tmplFilters && p.tmplFilters.length) {
            $(".ui-template", fil).on("change", function() {
              ...
              if (curtempl === "default") {
                ...
                $("#" + fid).jqFilter(
                  "addFilter",
                  p.tmplFilters[parseInt(curtempl, 10)]
                );
              }
              return false;
            });
          ...
          if (p.layer) {
            $.jgrid.createModal(
              ...
              typeof p.layer === "string"
                ...
                : p.layer,
              typeof p.layer === "string"
                ? { position: "relative", "font-size": fs }
                : { "font-size": fs }
            );
          } else {
            $.jgrid.createModal(
          ...
          if (p.searchOnEnter || p.closeOnEscape) {
            $("#" + $.jgrid.jqID(IDs.themodal)).keydown(function(e) {
              ...
              if (
                p.searchOnEnter &&
                e.which === 13 && // 13 === $.ui.keyCode.ENTER
                !$target.hasClass("add-group") &&
                !$target.hasClass("add-rule") &&
                !$target.hasClass("delete-group") &&
                !$target.hasClass("delete-rule") &&
                (!$target.hasClass("fm-button") || !$target.is("[id$=_query]"))
              ) {
                $("#" + fid + "_search").click();
                return false;
              }
              if (p.closeOnEscape && e.which === 27) {
                // 27 === $.ui.keyCode.ESCAPE
    ...
    filterInput: function(val, p) {
      ...
      return this.each(function() {
        ...
        var nm,
          sop,
          ruleGroup = '{"groupOp":"' + p.groupOp + '","rules":[',
          gi = 0,
          so;
        val += "";
        if ($t.p.datatype !== "local") {
        ...
        var bsr =
          $($t).triggerHandler("jqGridFilterInputBeforeSearch") === "stop"
            ? true
            : false;
        if (!bsr && $.isFunction(p.beforeSearch)) {
          bsr = p.beforeSearch.call($t);
      ...
      return this.each(function() {
        ...
        if (o.field && $t.p.data && $.isArray($t.p.data)) {
          ...
          if (result && result.length) {
            res = $("#gsh_" + $t.p.id + "_" + o.field).find(
  ...
  $.jgrid.extend({
    editGridRow: function(rowid, p) {
      ...
      return this.each(function() {
        ...
        var gID = $t.p.id,
          ...
          frmoper,
          templ =
            typeof rp_ge[$t.p.id].template === "string" &&
            rp_ge[$t.p.id].template.length > 0,
          errors = $.jgrid.getRegional(this, "errors");
              ...
              if (celm.length) {
                ...
                $.each($t.p.colModel, function() {
                  if (
                    ...
                    try {
                      postdata[nm] = this.editoptions.custom_value.call(
                        $t,
                        $("#" + $.jgrid.jqID(nm), frmtb),
                        "get"
                      );
                      ...
                      if (e === "e1") {
                        $.jgrid.info_dialog(
                          ...
                          "function 'custom_value' " +
                            rp_ge[$(this)[0]].p.msg.novalue,
                          rp_ge[$(this)[0]].p.bClose,
          ...
          $(obj.p.colModel).each(function(i) {
            ...
            if (
                ...
                if (nm === obj.p.ExpandColumn && obj.p.treeGrid === true) {
                  ...
                  try {
                    tmp = $.unformat.call(
                      ...
                      { rowId: rowid, colModel: this },
                      i
                    );
                  } catch (_) {
                    tmp =
                      this.edittype && this.edittype === "textarea"
                        ? $(
                            "td[role='gridcell']:eq(" + i + ")",
                            obj.rows[ind]
                          ).text()
              ...
              if (rowid === "_empty" && opt.defaultValue) {
                tmp = $.isFunction(opt.defaultValue)
                  ? opt.defaultValue.call($t)
                  : opt.defaultValue;
              }
              if (!this.edittype) {
                this.edittype = "text";
              ...
              elc = $.jgrid.createEl.call(
                $t,
                this.edittype,
                opt,
                tmp,
                false,
                $.extend({}, $.jgrid.ajaxOptions, obj.p.ajaxSelectOptions || {})
              );
              ...
              if (templ) {
                ...
                if (trdata.length === 0) {
                  if (maxcols > 1) {
                    trdata = $("<tr rowpos='" + rp + "'></tr>")
                      .addClass("FormData")
                      .attr("id", "tr_" + nm);
                  } else {
                    trdata = $("<tr " + dc + " rowpos='" + rp + "'></tr>")
                      .addClass("FormData")
                      .attr("id", "tr_" + nm);
                  }
                  $(trdata).append(tmpl);
                ...
                $("td:eq(" + (cp - 2) + ")", trdata[0]).html(
                  "<label for='" +
                    nm +
                    "'>" +
                    (frmopt.label === undefined
                      ? obj.p.colNames[i]
                      : frmopt.label) +
                    "</label>"
                );
                $("td:eq(" + (cp - 1) + ")", trdata[0])
                  .append(frmopt.elmprefix)
                  .append(elc)
                  .append(frmopt.elmsuffix);
                if (maxcols > 1 && hc) {
            ...
            $(cm).each(function() {
              ...
              if (fld && fld.length && fld[0] !== null) {
                ...
                if (
                  ...
                  vl = $.isFunction(opt.defaultValue)
                    ? opt.defaultValue.call($t)
                    : opt.defaultValue;
                  if (fld[0].type === "checkbox") {
                    vlc = vl.toLowerCase();
                    if (
                      vlc.search(/(false|f|0|no|n|off|undefined)/i) < 0 &&
                      vlc !== ""
                    ) {
                      fld[0].checked = true;
                ...
                if (
                  rp_ge[$t.p.id].checkOnSubmit === true ||
                  rp_ge[$t.p.id].checkOnUpdate
                ) {
                  $t.p.savedData[nm] = vl;
                }
              ...
              switch (cm[i].edittype) {
                case "select":
                  ...
                  $("#" + nm + " option", fmid).each(function() {
                    if (
                      !cm[i].editoptions.multiple &&
                      ($.trim(tmp) === $.trim($(this).text()) ||
                        opv[0] === $.trim($(this).text()) ||
                        opv[0] === $.trim($(this).val()))
                    ) {
                      this.selected = true;
                    } else if (cm[i].editoptions.multiple) {
                      if (
                        $.inArray($.trim($(this).text()), opv) > -1 ||
                        $.inArray($.trim($(this).val()), opv) > -1
                      ) {
                        this.selected = true;
                      } else {
                        this.selected = false;
                ...
                case "checkbox":
                  ...
                  if (cm[i].editoptions && cm[i].editoptions.value) {
                    ...
                    tmp = tmp.toLowerCase();
                    if (
                      tmp.search(/(false|f|0|no|n|off|undefined)/i) < 0 &&
                      tmp !== ""
                    ) {
                      $("#" + nm, fmid)[$t.p.useProp ? "prop" : "attr"](
                        "checked",
                ...
                case "custom":
                  try {
                    if (
                      ...
                      cm[i].editoptions.custom_value.call(
                        $t,
                        $("#" + nm, fmid),
                        "set",
                        tmp
        ...
        function postIt() {
          ...
          var retvals = $($t).triggerHandler("jqGridAddEditBeforeCheckValues", [
            postdata,
            $(frmgr),
            frmoper
          ]);
          ...
          if ($.isFunction(rp_ge[$t.p.id].beforeCheckValues)) {
            retvals = rp_ge[$t.p.id].beforeCheckValues.call(
              $t,
              postdata,
              $(frmgr),
              frmoper
          ...
          setNulls();
          if (ret[0]) {
            onCS = $($t).triggerHandler("jqGridAddEditClickSubmit", [
              rp_ge[$t.p.id],
              postdata,
              frmoper
            ]);
            if (
              ...
              onCS =
                rp_ge[$t.p.id].onclickSubmit.call(
                  $t,
                  rp_ge[$t.p.id],
                  postdata,
                  frmoper
            ...
            ret = $($t).triggerHandler("jqGridAddEditBeforeSubmit", [
              postdata,
              $(frmgr),
              frmoper
            ]);
            ...
            if (ret[0] && $.isFunction(rp_ge[$t.p.id].beforeSubmit)) {
              ret = rp_ge[$t.p.id].beforeSubmit.call(
                $t,
                postdata,
                $(frmgr),
                frmoper
          ...
          if (ret[0] && !rp_ge[$t.p.id].processing) {
            ...
            postdata[oper] =
              $.trim(postdata[$t.p.id + "_id"]) === "_empty"
                ? opers.addoper
                : opers.editoper;
            ...
            if ($t.p.treeGrid === true) {
              if (postdata[oper] === opers.addoper) {
                ...
                var tr_par_id =
                  $t.p.treeGridModel === "adjacency"
                    ? $t.p.treeReader.parent_id_field
                    : "parent_id";
            ...
            var ajaxOptions = $.extend(
              {
                ...
                data: $.isFunction(rp_ge[$t.p.id].serializeEditData)
                  ? rp_ge[$t.p.id].serializeEditData.call($t, postdata)
                  : postdata,
                complete: function(data, status) {
                  ...
                  if (data.status >= 300 && data.status !== 304) {
                    ret[0] = false;
                    ret[1] = $($t).triggerHandler(
                      "jqGridAddEditErrorTextFormat",
                      [data, frmoper]
                    );
                    if ($.isFunction(rp_ge[$t.p.id].errorTextFormat)) {
                      ret[1] = rp_ge[$t.p.id].errorTextFormat.call(
                        $t,
                        data,
                        frmoper
                      );
                    ...
                    ret = $($t).triggerHandler("jqGridAddEditAfterSubmit", [
                      data,
                      postdata,
                      frmoper
                    ]);
                    ...
                    if (ret[0] && $.isFunction(rp_ge[$t.p.id].afterSubmit)) {
                      ret = rp_ge[$t.p.id].afterSubmit.call(
                        $t,
                        data,
                        postdata,
                        frmoper
                  ...
                  if (ret[0] === false) {
                    ...
                    if (postdata[oper] === opers.addoper) {
                      ...
                      if (
                        postdata[idname] == null ||
                        postdata[idname] === $t.p.idPrefix + "_empty" ||
                        postdata[idname] === ""
                      ) {
                    ...
                    if (
                      ...
                      $._data($($t)[0], "events").hasOwnProperty(
                        "jqGridAddEditAfterComplete"
                      )
                    ) {
                      copydata = data;
                      setTimeout(function() {
                        $($t).triggerHandler("jqGridAddEditAfterComplete", [
                          copydata,
                          postdata,
                          $(frmgr),
                          frmoper
                        ...
                        try {
                          rp_ge[$t.p.id].afterComplete.call(
                            $t,
                            copydata,
                            postdata,
                            $(frmgr),
                    ...
                    if (
                      ...
                      if ($t.p.savedData[$t.p.id + "_id"] !== "_empty") {
                        for (key in $t.p.savedData) {
                          if (
                            $t.p.savedData.hasOwnProperty(key) &&
                            postdata[key]
                          ) {
            ...
            if (ret[0]) {
              if (rp_ge[$t.p.id].useDataProxy) {
                ...
                if (dpret[0] === false) {
                  ...
                  if (
                    ajaxOptions.data.oper === opers.addoper &&
                  ...
                  if (
                    ajaxOptions.data.oper === opers.editoper &&
        ...
        function compareData(nObj, oObj) {
          ...
          ret = !(
            $.isPlainObject(nObj) &&
            $.isPlainObject(oObj) &&
            Object.getOwnPropertyNames(nObj).length ===
              Object.getOwnPropertyNames(oObj).length
          );
          if (!ret) {
            for (key in oObj) {
        ...
        function updateNav(cr, posarr) {
          ...
          if (cr === 0) {
            ...
            posarr[1][cr - 1] !== undefined &&
            $("#" + $.jgrid.jqID(posarr[1][cr - 1])).hasClass(
        ...
        var dh = isNaN(rp_ge[$(this)[0].p.id].dataheight)
            ? rp_ge[$(this)[0].p.id].dataheight
            : rp_ge[$(this)[0].p.id].dataheight + "px",
          dw = isNaN(rp_ge[$(this)[0].p.id].datawidth)
            ? rp_ge[$(this)[0].p.id].datawidth
            : rp_ge[$(this)[0].p.id].datawidth + "px",
          frm = $(
        ...
        if (templ) {
          ...
          tbl = $(
            "<table id='" +
              frmtborg +
              "' class='EditTable ui-common-table'><tbody></tbody></table>"
          );
          frmtb2 = frmtb + "_2";
        }
        ...
        // topinfo
        $(frm).append(
          "<div class='tinfo topinfo'>" + rp_ge[$t.p.id].topinfo + "</div>"
        );
        ...
        showFrm = $($t).triggerHandler("jqGridAddEditBeforeInitData", [
          frm,
          frmoper
        ]);
        if (showFrm === undefined) {
        ...
        var bP =
          ...
          bN =
            ...
            "'></span></a>",
          bS =
            "<a id='sData' class='fm-button " +
            commonstyle.button +
            "'>" +
            p.bSubmit +
            "</a>",
          bC =
            "<a id='cData' class='fm-button " +
            commonstyle.button +
            "'>" +
            p.bCancel +
            "</a>",
          user_buttons = $.isArray(rp_ge[$t.p.id].buttons)
            ? $.jgrid.buildButtons(rp_ge[$t.p.id].buttons, bS + bC, commonstyle)
            : bS + bC;
        var bt =
          "<table style='height:auto' class='EditTable ui-common-table' id='" +
          frmtborg +
          "_2'><tbody><tr><td colspan='2'><hr class='" +
          commonstyle.content +
          "' style='margin:1px'/></td></tr><tr id='Act_Buttons'><td class='navButton'>" +
          (rtlb ? bN + bP : bP + bN) +
          "</td><td class='EditButton'>" +
          user_buttons +
          "</td></tr>";
        ...
        $(frm).append(
          "<div class='binfo topinfo bottominfo'>" +
        ...
        $("#" + $.jgrid.jqID(IDs.themodal)).keydown(function(e) {
          ...
          if (
            rp_ge[$t.p.id].savekey[0] === true &&
            e.which === rp_ge[$t.p.id].savekey[1]
          ) {
            // save
        ...
        if (p.checkOnUpdate) {
          ...
          $("a.ui-jqdialog-titlebar-close", "#" + $.jgrid.jqID(IDs.themodal))
            .off("click")
            .click(function() {
              if (!checkUpdates()) {
                return false;
        ...
        if (rp_ge[$t.p.id].checkOnSubmit || rp_ge[$t.p.id].checkOnUpdate) {
          bS =
            "<a id='sNew' class='fm-button " +
            commonstyle.button +
            "' style='z-index:1002'>" +
            p.bYes +
            "</a>";
          bN =
            "<a id='nNew' class='fm-button " +
            commonstyle.button +
            "' style='z-index:1002;margin-left:5px'>" +
            p.bNo +
            "</a>";
          bC =
            "<a id='cNew' class='fm-button " +
            commonstyle.button +
            "' style='z-index:1002;margin-left:5px;'>" +
            p.bExit +
            "</a>";
          ...
          $("#" + IDs.themodal).append(
            "<div class='" +
              ...
              "</div><div class='confirm ui-jqconfirm " +
              commonstyle.content +
              "' style='z-index:" +
              (zI + 1) +
              "'>" +
        ...
        // here initform
        $($t).triggerHandler("jqGridAddEditInitializeForm", [
          $(frmgr),
          frmoper
        ]);
        ...
        $($t).triggerHandler("jqGridAddEditBeforeShowForm", [
          $(frmgr),
          frmoper
        ]);
        ...
        $("#nData", frmtb2).click(function() {
          ...
          if (npos[0] !== -1 && npos[1][npos[0] + 1]) {
            $($t).triggerHandler("jqGridAddEditClickPgButtons", [
              "next",
              $(frmgr),
              npos[1][npos[0]]
            ]);
            ...
            if ($.isFunction(p.onclickPgButtons)) {
              nposret = p.onclickPgButtons.call(
                $t,
                "next",
                $(frmgr),
                npos[1][npos[0]]
            ...
            if (
              $("#" + $.jgrid.jqID(npos[1][npos[0] + 1])).hasClass(
                commonstyle.disabled
              )
            ) {
              return false;
            ...
            $($t).triggerHandler("jqGridAddEditAfterClickPgButtons", [
              "next",
              $(frmgr),
              npos[1][npos[0]]
            ]);
            if ($.isFunction(p.afterclickPgButtons)) {
              p.afterclickPgButtons.call(
                $t,
                "next",
                $(frmgr),
                npos[1][npos[0] + 1]
        ...
        $("#pData", frmtb2).click(function() {
          ...
          if (ppos[0] !== -1 && ppos[1][ppos[0] - 1]) {
            ...
            var pposret;
            if ($.isFunction(p.onclickPgButtons)) {
              pposret = p.onclickPgButtons.call(
                $t,
                "prev",
                $(frmgr),
                ppos[1][ppos[0]]
            ...
            if (
              $("#" + $.jgrid.jqID(ppos[1][ppos[0] - 1])).hasClass(
                commonstyle.disabled
              )
            ) {
              return false;
    ...
    viewGridRow: function(rowid, p) {
      ...
      return this.each(function() {
        ...
        function focusaref() {
          //Sfari 3 issues
          if (
            rp_ge[$t.p.id].closeOnEscape === true ||
        ...
        function createData(rowid, obj, tb, maxcols) {
          var nm,
            ...
            i,
            tdtmpl =
              "<td class='CaptionTD form-view-label " +
              commonstyle.content +
              "' width='" +
              p.labelswidth +
              "'></td><td class='DataTD form-view-data ui-helper-reset " +
              commonstyle.content +
              "'></td>",
            tmpl = "",
            tdtmpl2 =
              "<td class='CaptionTD form-view-label " +
              commonstyle.content +
              "'></td><td class='DataTD form-view-data " +
              commonstyle.content +
              "'></td>",
        ...
        var dh = isNaN(rp_ge[$(this)[0].p.id].dataheight)
          ...
          tbl = $(
            "<table id='" +
              frmtb_id +
              "' class='EditTable ViewTable'><tbody></tbody></table>"
          );
        $($t.p.colModel).each(function() {
          var fmto = this.formoptions;
          maxCols = Math.max(maxCols, fmto ? fmto.colpos || 0 : 0);
        ...
        var rtlb = $t.p.direction === "rtl" ? true : false,
          ...
          bC =
            ...
            "</a>",
          user_buttons = $.isArray(rp_ge[$t.p.id].buttons)
            ? $.jgrid.buildButtons(rp_ge[$t.p.id].buttons, bC, commonstyle)
            : bC;
        if (maxRows > 0) {
          var sd = [];
          $.each($(tbl)[0].rows, function(i, r) {
        ...
        $(
          ".fm-button:not(." + commonstyle.disabled + ")",
          "#" + frmtb + "_2"
        ).hover(
          function() {
            $(this).addClass(commonstyle.hover);
        ...
        $("#nData", "#" + frmtb + "_2").click(function() {
          ...
          npos[0] = parseInt(npos[0], 10);
          if (npos[0] !== -1 && npos[1][npos[0] + 1]) {
            $($t).triggerHandler("jqGridViewRowClickPgButtons", [
              "next",
              $("#" + frmgr),
              npos[1][npos[0]]
            ]);
            if ($.isFunction(p.onclickPgButtons)) {
              p.onclickPgButtons.call(
                $t,
                "next",
                $("#" + frmgr),
                npos[1][npos[0]]
            ...
            $($t).jqGrid("setSelection", npos[1][npos[0] + 1]);
            $($t).triggerHandler("jqGridViewRowAfterClickPgButtons", [
              "next",
              $("#" + frmgr),
              npos[1][npos[0] + 1]
            ]);
    ...
    delGridRow: function(rowids, p) {
      ...
      return this.each(function() {
        ...
        if ($("#" + $.jgrid.jqID(IDs.themodal))[0] !== undefined) {
          showFrm = $($t).triggerHandler("jqGridDelRowBeforeInitData", [
            $("#" + dtbl)
          ]);
          if (showFrm === undefined) {
            showFrm = true;
          ...
          var dh = isNaN(rp_ge[$t.p.id].dataheight)
              ? rp_ge[$t.p.id].dataheight
          ...
          // error data
          tbl +=
            "<tr id='DelError' style='display:none'><td class='" +
            commonstyle.error +
            "'></td></tr>";
          tbl +=
          ...
          tbl += "</tbody></table></div>";
          var bS =
              "<a id='dData' class='fm-button " +
              commonstyle.button +
              "'>" +
              p.bSubmit +
              "</a>",
            bC =
              "<a id='eData' class='fm-button " +
              commonstyle.button +
              "'>" +
              p.bCancel +
              "</a>",
          ...
          tbl +=
            "<table class='EditTable ui-common-table' id='" +
            dtbl +
            "_2'><tbody><tr><td><hr class='" +
            commonstyle.content +
            "' style='margin:1px'/></td></tr><tr><td class='DelButton EditButton'>" +
            user_buttons +
            "</td></tr></tbody></table>";
          p.gbox = "#gbox_" + $.jgrid.jqID(gID);
          $.jgrid.createModal(
          ...
          p.delicon = $.extend(
            [true, "left", styles.icon_del],
            rp_ge[$t.p.id].delicon
          );
          p.cancelicon = $.extend(
            [true, "left", styles.icon_cancel],
          ...
          $("#dData", "#" + dtbl + "_2").click(function() {
            ...
            onCS = {};
            onCS = $($t).triggerHandler("jqGridDelRowClickSubmit", [
              rp_ge[$t.p.id],
              postdata
            ]);
            if (
              ...
              var ajaxOptions = $.extend(
                {
                  url:
                    rp_ge[$t.p.id].url ||
                    $($t).jqGrid("getGridParam", "editurl"),
                  type: rp_ge[$t.p.id].mtype,
                  data: $.isFunction(rp_ge[$t.p.id].serializeDelData)
                    ? rp_ge[$t.p.id].serializeDelData.call($t, postd)
                    : postd,
                  complete: function(data, status) {
                    var i;
                    $("#dData", "#" + dtbl + "_2").removeClass(
                      commonstyle.active
                    );
                    if (data.status >= 300 && data.status !== 304) {
                      ret[0] = false;
                      ret[1] = $($t).triggerHandler(
                        "jqGridDelRowErrorTextFormat",
                        [data]
                      );
                      if ($.isFunction(rp_ge[$t.p.id].errorTextFormat)) {
                      ...
                      // data is posted successful
                      // execute aftersubmit with the returned data from server
                      ret = $($t).triggerHandler("jqGridDelRowAfterSubmit", [
                        data,
                        postd
                      ]);
                      if (ret === undefined) {
                    ...
                    if (ret[0] === false) {
                      ...
                      if (
                        ...
                        var copydata = data;
                        setTimeout(function() {
                          $($t).triggerHandler("jqGridDelRowAfterComplete", [
                            copydata,
                            postd
                          ]);
                          try {
        ...
        if (rp_ge[$t.p.id].closeOnEscape === true) {
          setTimeout(function() {
            $(".ui-jqdialog-titlebar-close", "#" + $.jgrid.jqID(IDs.modalhead))
              .attr("tabindex", "-1")
              .focus();
          }, 0);
        }
    ...
    navGrid: function(elem, p, pEdit, pAdd, pDel, pSearch, pView) {
      ...
      return this.each(function() {
        ...
        for (i = 0; i < clone; i++) {
          var tbd,
            navtbl = $(
              "<table class='ui-pg-table navtable ui-common-table'><tbody><tr></tr></tbody></table>"
            ),
            sep =
              "<td class='ui-pg-button " +
              commonstyle.disabled +
          ...
          if (o.add) {
            tbd = $(
              "<td class='ui-pg-button " + commonstyle.cornerall + "'></td>"
            );
            $(tbd).append(
              "<div class='ui-pg-div'><span class='" +
                commonstyle.icon_base +
                " " +
                o.addicon +
                "'></span>" +
          ...
          $("body").append(
            "<div id='testpg2' class='ui-jqgrid " +
              $.jgrid.styleUI[currstyle].base.entrieBox +
              "' style='font-size:" +
              tdw +
              ";visibility:hidden;' ></div>"
          );
      ...
      return this.each(function() {
        ...
        if (findnav) {
          ...
          if (p.buttonicon.toString().toUpperCase() === "NONE") {
            $(tbd)
              .addClass("ui-pg-button " + cornerall)
              .append("<div class='ui-pg-div'>" + p.caption + "</div>");
          } else {
            $(tbd)
              .addClass("ui-pg-button " + cornerall)
              .append(
          ...
          if (findnav) {
            var id = $(findnav).val(),
              ...
              item = $(
                '<li class="ui-menu-item" role="presentation"><a class="' +
                  cornerall +
                  ' g-menu-item" tabindex="0" role="menuitem" id="' +
                  eid +
                  '">' +
                  (p.caption || p.title) +
                  "</a></li>"
      ...
      return this.each(function() {
        ...
        if (findnav) {
          ...
          sep =
            "<li class='ui-menu-item " +
            commonstyle.disabled +
            "' style='width:100%' role='presentation'><hr class='ui-separator-li'></li>";
          if (findnav) {
            id = $(findnav).val();
            if (id) {
              if (p.position === "first") {
    ...
    _buildNavMenu: function(
      ...
      return this.each(function() {
        var $t = this,
          ...
          bt = p.navButtonText
            ? p.navButtonText
            : regional.selectcaption || "Actions",
          act =
          ...
          _buildMenu = function() {
            var fs = $(".ui-jqgrid").css("font-size") || "11px",
              ...
              str = $(
                '<ul id="' +
                  mid +
                  '" class="ui-nav-menu modal-content" role="menu" tabindex="0" style="display:none;font-size:' +
                  fs +
                  '"></ul>'
              );
            if (p.add) {
              ...
              itm = $(
                '<li class="ui-menu-item" role="presentation"><a class="' +
                  commonstyle.cornerall +
                  ' g-menu-item" tabindex="0" role="menuitem" id="' +
                  eid +
                  '">' +
                  (p.addtext || p.addtitle) +
                  "</a></li>"
              ).click(function() {
                if (!$(this).hasClass(commonstyle.disabled)) {
            ...
            if (p.refresh) {
              ...
              itm = $(
                '<li class="ui-menu-item" role="presentation"><a class="' +
                  ...
                  eid +
                  '">' +
                  (p.refreshtext || p.refreshtitle) +
                  "</a></li>"
              ).click(function() {
                if (!$(this).hasClass(commonstyle.disabled)) {
                  if ($.isFunction(p.beforeRefresh)) {
                    p.beforeRefresh.call($t);
                  }
                  $t.p.search = false;
                  $t.p.resetsearch = true;
                  try {
                  ...
                  if ($.isFunction(p.afterRefresh)) {
                    p.afterRefresh.call($t);
                  }
                  $(str).hide();
                }
                return false;
        ...
        if (rowdata) {
          for (i in rowdata) {
            if (rowdata.hasOwnProperty(i)) {
              if (
                $("[name=" + $.jgrid.jqID(i) + "]", formid).is("input:radio") ||
                $("[name=" + $.jgrid.jqID(i) + "]", formid).is("input:checkbox")
              ) {
    ...
    groupingSetup: function() {
      return this.each(function() {
        ...
        if (grp !== null && (typeof grp === "object" || $.isFunction(grp))) {
          if (!grp.plusicon) {
            grp.plusicon = classes.icon_plus;
          }
          if (!grp.minusicon) {
            grp.minusicon = classes.icon_minus;
          }
          if (!grp.groupField.length) {
            ...
            for (i = 0; i < grp.groupField.length; i++) {
              ...
              if (grp.groupColumnShow[i] === true) {
                ...
                grp.visibiltyOnNextGrouping[i] = $(
                  "#" + $.jgrid.jqID($t.p.id + "_" + grp.groupField[i])
                ).is(":visible");
            ...
            if (grp.hideFirstGroupCol) {
              if (
                $.isArray(grp.formatDisplayField) &&
                !$.isFunction(grp.formatDisplayField[0])
              ) {
            ...
            for (j = 0, cml = cm.length; j < cml; j++) {
              ...
              if (cm[j].summaryType) {
                if (cm[j].summaryDivider) {
                  grp.summary.push({
                    decimalPlaces:
                      cm[j].formatoptions != undefined
                        ? cm[j].formatoptions.decimalPlaces
                        : null,
                    pre:
                      cm[j].formatoptions != undefined
                        ? cm[j].formatoptions.prefix
                        : null,
                    p: j,
                    nm: cm[j].name,
                    st: cm[j].summaryType,
        ...
        for (i = 0; i < grlen; i++) {
          ...
          if (v !== undefined) {
            if (irow === 0) {
              ...
              if (
                typeof v !== "object" &&
                ($.isArray(grp.isInTheSameGroup) &&
                $.isFunction(grp.isInTheSameGroup[i])
                  ? !grp.isInTheSameGroup[i].call(
                      $t,
                      grp.lastvalues[i],
                      v,
                      i,
                ...
                $.each(grp.counters[i].summary, sumGroups);
                grp.groups[grp.counters[i].pos].summary =
    ...
    groupingToggle: function(hid) {
      this.each(function() {
        ...
        var uid = strpos.join("_"),
          ...
          tarspan = $(
            "#" + $.jgrid.jqID(hid) + " span." + "tree-wrap-" + $t.p.direction
          ),
        ...
        if (tarspan.hasClass(minus)) {
          ...
          if (r) {
            ...
            while (r) {
              ...
              skip =
                $(r).hasClass("ui-subgrid") && $(r).hasClass("ui-sg-collapsed");
              if (itemGroupingLevel !== undefined) {
                ...
                if (itemGroupingLevel === num + 1) {
                  if (!skip) {
                    $(r)
                      .show()
                      .find(">td>span." + "tree-wrap-" + $t.p.direction)
                      .removeClass(minus)
                      .addClass(plus);
                    if (frz) {
                      $(r2)
    ...
    groupingRender: function(grdata, colspans, page, rn) {
      return this.each(function() {
          ...
          for (k = foffset; k < colspans; k++) {
            ...
            $.each(fdata.summary, function() {
              if (this.nm === cm[k].name) {
                ...
                if (
                  typeof this.st === "string" &&
                  this.st.toLowerCase() === "avg"
                ) {
                  if (this.sd && this.vd) {
        ...
        $.each(grp.groups, function(i, n) {
          if (grp._locgr) {
            if (
              !(n.startRow + n.cnt > (page - 1) * rn && n.startRow < page * rn)
            ) {
              return true;
            }
          }
          ...
          hid = clid + "_" + i;
          icon =
            "<span style='cursor:pointer;margin-right:8px;margin-left:5px;' class='" +
            common.icon_base +
            " " +
            pmrtl +
            "' onclick=\"jQuery('#" +
          ...
          try {
            if (
              $.isArray(grp.formatDisplayField) &&
              $.isFunction(grp.formatDisplayField[n.idx])
            ) {
              gv = grp.formatDisplayField[n.idx].call(
                $t,
                n.displayValue,
                n.value,
                $t.p.colModel[cp[n.idx]],
                n.idx,
                grp
              );
            } else {
          ...
          if (
            !(typeof grpTextStr === "string" || typeof grpTextStr === "number")
          ) {
            grpTextStr = gv;
          }
          if (grp.groupSummaryPos[n.idx] === "header") {
            ...
            for (var myCont = 1; myCont <= colspans; myCont++) {
              str +=
                "<td class='sumas' style=\"padding-left:" +
                n.idx * 12 +
                "px;" +
                '">' +
                "</td>";
          ...
          if (leaf) {
            var gg = grp.groups[i + 1],
              ...
              sgr = n.startRow,
                ...
                gg !== undefined
                  ? gg.startRow
                  : grp.groups[i].startRow + grp.groups[i].cnt;
            if (grp._locgr) {
              offset = (page - 1) * rn;
    ...
    groupingGroupBy: function(name, options) {
      return this.each(function() {
        ...
        for (i = 0; i < name.length; i++) {
          grp.visibiltyOnNextGrouping[i] = $(
            "#" + $.jgrid.jqID($t.p.id) + "_" + $.jgrid.jqID(name[i])
          ).is(":visible");
        }
    ...
    setGroupHeaders: function(o) {
      ...
      return this.each(function() {
        ...
        if ($firstHeaderRow[0] === undefined) {
          $firstHeaderRow = $("<tr>", { role: "row", "aria-hidden": "true" })
        ...
        for (i = 0; i < cml; i++) {
          ...
          $("<th>", { role: "gridcell" })
            .css(thStyle)
            .addClass("ui-first-th-" + ts.p.direction)
          ...
          if (iCol >= 0) {
            ...
            for (
              cVisibleColumns = 0, iCol = 0;
              iCol < numberOfColumns && i + iCol < cml;
              iCol++
            ) {
        ...
        if (o.useColSpanStyle) {
          ...
          $htable.find("div.ui-jqgrid-sortable").each(function() {
            ...
            if (
              $parent.is(":visible") &&
              $parent.is(":has(span.ui-jqgrid-resize)")
            ) {
              // minus 4px from the margins of the resize markers
              $ts.css(
                "top",
                ($parent.height() - $ts.outerHeight()) / 2 - 4 + "px"
              );
            }
          });
        }
  ...
  $.extend($.jgrid, {
    saveState: function(jqGridId, o) {
      ...
      if (o.saveData) {
        data = $($t.grid.bDiv)
          .find(".ui-jqgrid-btable tbody:first")
          .html();
      ...
      if (o.debug) {
        $("#gbox_tree").prepend(
          '<a id="link_save" target="_blank" download="jqGrid_dump.txt">Click to save Dump Data</a>'
        );
        var temp = [],
          file,
          properties = {},
      ...
      if (ret && $.type(ret) === "object") {
            ...
            $(grid[0].rows[i].cells[expCol])
              ...
              .on("click", function(e) {
                var target = e.target || e.srcElement,
                  ind2 = $.jgrid.stripPref(
                    ret.idPrefix,
                    $(target, grid[0].rows).closest("tr.jqgrow")[0].id
                  ),
        ...
        if (ret.multiselect) {
          $.each(ret.selarrrow, function() {
            $("#jqg_" + jqGridId + "_" + this)[ret.useProp ? "prop" : "attr"](
              "checked",
              "checked"
            );
          });
    ...
    setRegional: function(jqGridId, options) {
      ...
      if (i !== -1 && nav) {
        $("#" + jqGridId + " tbody tr").each(function() {
          ...
          $(td)
            .find(".ui-inline-edit")
            .attr("title", nav.edittitle);
          $(td)
            .find(".ui-inline-del")
            .attr("title", nav.deltitle);
          $(td)
            .find(".ui-inline-save")
            .attr("title", nav.savetitle);
          $(td)
            .find(".ui-inline-cancel")
            .attr("title", nav.canceltitle);
        });
      }
      try {
    ...
    jqGridImport: function(jqGridId, o) {
      ...
      var $t =
        (jqGridId.indexOf("#") === 0 ? "" : "#") + $.jgrid.jqID(jqGridId);
      var xmlConvert = function(xml, o) {
        ...
        if ($.grid.xmlToJSON) {
          ...
          if (xmldata) {
            ...
            $($t)
              .jqGrid(jstr1)
              .jqGrid("setGridParam", { datatype: svdatatype });
          } else {
            setTimeout(function() {
              $($t).jqGrid(jstr1);
            }, 0);
  ...
  $.jgrid.extend({
    ...
    editRow: function(
      ...
      return this.each(function() {
        ...
        bfer = $.isFunction(o.beforeEditRow)
          ? o.beforeEditRow.call($t, o, rowid)
          : undefined;
        if (bfer === undefined) {
          bfer = true;
        }
        ...
        if (editable === "0" && !$(ind).hasClass("not-editable-row")) {
          ...
          $('td[role="gridcell"]', ind).each(function(i) {
            ...
            if (treeg) {
              ...
              try {
                ...
                tmp =
                  cm[i].edittype && cm[i].edittype === "textarea"
                    ? $(this).text()
                    : $(this).html();
              }
            }
          ...
          if (cnt > 0) {
            ...
            if (o.focusField) {
              if (
                typeof o.focusField === "number" &&
                parseInt(o.focusField, 10) <= cm.length
              ) {
                focus = o.focusField;
              }
              setTimeout(function() {
                var fe = $("td:eq(" + focus + ") :input:visible", ind).not(
                  ":disabled"
                );
                if (fe.length > 0) {
                  fe.focus();
    ...
    saveRow: function(
      ...
      var success = false,
        ...
        nullIfEmpty = false,
        error = $.trim(
          $($t).jqGrid("getStyleUI", $t.p.styleUI + ".common", "error", true)
        );
      if (!$t.grid) {
        return success;
      }
      ...
      var errors = $.jgrid.getRegional($t, "errors"),
        edit = $.jgrid.getRegional($t, "edit"),
        bfsr = $.isFunction(o.beforeSaveRow)
          ? o.beforeSaveRow.call($t, o, rowid)
          : undefined;
      if (bfsr === undefined) {
        bfsr = true;
      }
      ...
      if (editable === "1") {
        ...
        if (o.url === "clientArray") {
          ...
          var k,
            resp = $($t).jqGrid("setRowData", rowid, tmp);
          $(ind).attr("editable", "0");
          ...
          $.ajax(
            $.extend(
              {
                url: o.url,
                data: $.isFunction($t.p.serializeRowData)
                  ? $t.p.serializeRowData.call($t, tmp3)
                  : tmp3,
                type: o.mtype,
                async: false, //?!?
                complete: function(res, stat) {
                  ...
                  if (stat === "success") {
                    ...
                    if (ret === true) {
                      ...
                      $($t).triggerHandler("jqGridInlineErrorSaveRow", [
                        rowid,
                        res,
                        stat,
                        null,
                ...
                error: function(res, stat, err) {
                  ...
                  if ($.isFunction(o.errorfunc)) {
                    ...
                    try {
                      $.jgrid.info_dialog(
                        errors.errcap,
                        '<div class="' + error + '">' + rT + "</div>",
                        edit.bClose,
                        { buttonalign: "right", styleUI: $t.p.styleUI }
                      );
                    } catch (e) {
                      alert(rT);
                    }
    ...
    restoreRow: function(rowid, afterrestorefunc) {
      ...
      return this.each(function() {
        ...
        var bfcr = $.isFunction(o.beforeCancelRow)
          ? o.beforeCancelRow.call($t, o, rowid)
          : undefined;
        if (bfcr === undefined) {
          bfcr = true;
        }
        ...
        if (fr >= 0) {
          if ($.isFunction($.fn.datepicker)) {
            try {
              $("input.hasDatepicker", "#" + $.jgrid.jqID(ind.id)).datepicker(
                "hide"
              );
            } catch (e) {}
          }
          ...
          $t.p.savedRow.splice(fr, 1);
          if (
            $("#" + $.jgrid.jqID(rowid), "#" + $.jgrid.jqID($t.p.id)).hasClass(
              "jqgrid-new-row"
            )
          ) {
            setTimeout(function() {
    ...
    addRow: function(p) {
      ...
      return this.each(function() {
        ...
        var $t = this;
        $t.p.beforeAction = true;
        var bfar = $.isFunction(p.beforeAddRow)
          ? p.beforeAddRow.call($t, p.addRowParams)
          : undefined;
        if (bfar === undefined) {
          bfar = true;
        }
        ...
        $("#" + $.jgrid.jqID(p.rowID), "#" + $.jgrid.jqID($t.p.id)).addClass(
          "jqgrid-new-row"
        );
        if (p.useFormatter) {
          $(
            "#" + $.jgrid.jqID(p.rowID) + " .ui-inline-edit",
            "#" + $.jgrid.jqID($t.p.id)
          ).click();
        } else {
          var opers = $t.p.prmNames,
    ...
    columnChooser: function(opts) {
      var self = this,
        ...
        getMultiselectWidgetData = function($elem) {
          return (
            ...
            $elem.data("ui-multiselect") ||
            $elem.data("multiselect")
          );
        },
      ...
      function resize_select() {
        var widgetData = getMultiselectWidgetData(select),
          $thisDialogContent = widgetData.container.closest(
            ".ui-dialog-content"
          );
        if (
          $thisDialogContent.length > 0 &&
          typeof $thisDialogContent[0].style === "object"
        ) {
        ...
        widgetData.selectedList.height(
          Math.max(
            widgetData.selectedContainer.height() -
              widgetData.selectedActions.outerHeight() -
              1,
            1
          )
        );
        widgetData.availableList.height(
          Math.max(
            widgetData.availableContainer.height() -
              widgetData.availableActions.outerHeight() -
              1,
            1
          )
        );
      ...
      dopts = $.isFunction(opts.dlog_opts)
        ? opts.dlog_opts.call(self, opts)
        : opts.dlog_opts;
      call(opts.dlog, selector, dopts);
      mopts = $.isFunction(opts.msel_opts)
        ? opts.msel_opts.call(self, opts)
        : opts.msel_opts;
      ...
      listHeight = Math.max(
        multiselectData.selectedList.height(),
        multiselectData.availableList.height()
      );
            ...
            if ($t.p.rownumbers === true) {
              $("td.jqgrid-rownum", $t.rows).each(function(i) {
                $(this).html(
                  i +
                    1 +
                    (parseInt($t.p.page, 10) - 1) * parseInt($t.p.rowNum, 10)
                );
              });
            }
            if (opts._update_) {
    ...
    gridDnD: function(opts) {
      return this.each(function() {
        ...
        function updateDnD() {
          ...
          $("tr.jqgrow:not(.ui-draggable)", $t).draggable(
            $.isFunction(datadnd.drag)
              ? datadnd.drag.call($($t), datadnd)
              : datadnd.drag
          );
        }
              ...
              return $.extend(
                {
                  start: function(ev, ui) {
                    ...
                    for (i = 0; i < $.data($t, "dnd").connectWith.length; i++) {
                      if (
                        $($.data($t, "dnd").connectWith[i]).jqGrid(
                          "getGridParam",
                          "reccount"
                        ) === 0
                      ) {
              ...
              return $.extend(
                {
                  accept: function(d) {
                    ...
                    if (tid.length > 0 && $.data(tid[0], "dnd") !== undefined) {
                      ...
                      return $.inArray("#" + $.jgrid.jqID(target.id), cn) !== -1
                        ? true
                        : false;
                    }
                    return false;
                  ...
                  drop: function(ev, ui) {
                    ...
                    var accept = $(ui.draggable).attr("id"),
                      getdata = ui.draggable
                        .parent()
                        .parent()
                        .jqGrid("getRowData", accept),
                      keysd = [],
                    ...
                    if (!opts.dropbyname) {
                      var j,
                        ...
                        nm,
                        ki = 0;
                      var dropmodel = $("#" + $.jgrid.jqID(target.id)).jqGrid(
                        "getGridParam",
                        "colModel"
                      );
                      try {
                    ...
                    if (
                      $.data(tid[0], "dnd").beforedrop &&
                      $.isFunction($.data(tid[0], "dnd").beforedrop)
                    ) {
                      //parameters to this callback - event, element, data to be inserted, sender, reciever
                      // should return object which will be inserted into the reciever
                      var datatoinsert = $.data(tid[0], "dnd").beforedrop.call(
                        target,
                        ev,
                        ui,
                        getdata,
                    ...
                    if (opts.ondrop && $.isFunction(opts.ondrop)) {
                      opts.ondrop.call(target, ev, ui, getdata);
                    }
                  }
                },
                opts.drop_opts || {}
        ...
        for (i = 0; i < opts.connectWith.length; i++) {
          ...
          $(cn)
            ...
            .droppable(
              $.isFunction(opts.drop) ? opts.drop.call($($t), opts) : opts.drop
            );
        }
      });
    },
  ...
  $.jgrid.extend({
      ...
      this.each(function() {
        ...
        function agregateFunc(row, aggr, value, curr) {
          ...
          for (j = 0; j < jv; j++) {
            ...
            for (i = 0; i < arrln; i++) {
              swapstr =
                typeof aggr[i].aggregator === "string"
                  ? aggr[i].aggregator
                  : "cust";
              ...
              if (value == null) {
                ...
                vl = label;
                swapvals[0] =
                  aggr[i].label || swapstr + " " + $.trim(aggr[i].member);
              } else {
                ...
                try {
                  label =
                    arrln === 1
                      ? mainval + vl
                      : mainval + vl + "_" + swapstr + "_" + String(i);
                } catch (e) {}
                swapvals[j] = value[j];
              }
              ...
              curr[label] = tmpmember[label] = calculation(
                aggr[i].aggregator,
                curr[label],
                aggr[i].member,
                row,
            ...
            mainval +=
              value && value[j] != null ? value[j].replace(/\s+/g, "") : "";
          ...
          if (!newObj) {
            ...
            if (ylen >= 1) {
              ...
              for (k = 0; k < ylen; k++) {
                ...
                if (
                  o.yDimension[k].converter &&
                  $.isFunction(o.yDimension[k].converter)
                ) {
                  yValue[k] = o.yDimension[k].converter.call(
                    this,
                    yValue[k],
                    xValue,
                    yValue
          ...
          for (key in items) {
            ...
            if (items.hasOwnProperty(key)) {
              ...
              if (typeof items[key] !== "object") {
                ...
                if (key === "level") {
                  if (lastval[items.level] === undefined) {
                    ...
                    if (
                      items.level > 0 &&
                      items.text.indexOf("_r_Totals") === -1
                    ) {
                  ...
                  if (
                    ...
                    if (items.level > 0) {
                      ...
                      var collen =
                          headers[items.level - 1].groupHeaders.length - 1,
                      ...
                      if (items.level - 1 === (o.rowTotals ? 1 : 0)) {
                        if (collen > 0) {
                          ...
                          for (var kk = 0; kk < collen; kk++) {
                            l1 +=
                              headers[items.level - 1].groupHeaders[kk]
                                .numberOfColumns;
                          }
                          if (l1) {
                            colpos = l1 + xlen;
                      ...
                      if (columns[colpos]) {
                        headers[items.level - 1].groupHeaders[
                          collen
                        ].startColumnName = columns[colpos].name;
                        headers[items.level - 1].groupHeaders[
                          collen
                        ].numberOfColumns = columns.length - colpos;
                      }
                      initColLen = columns.length;
                    }
                  }
                      ...
                      if (items.fields.hasOwnProperty(l)) {
                        if (ll === 1) {
                          headers[ylen - 1].groupHeaders.push({
                            startColumnName: l,
                            numberOfColumns: 1,
                            titleText: items.label || items.text
                          });
                        }
                        ll++;
                      }
                    }
                    headers[ylen - 1].groupHeaders[
                      headers[ylen - 1].groupHeaders.length - 1
                    ].numberOfColumns = ll - 1;
                  } else {
              ...
              if (key === "level") {
                if (
                  items.level > 0 &&
                  (items.level === (ylen === 0 ? items.level : ylen) ||
                    lastval[items.level].indexOf("_r_Totals") !== -1)
                ) {
                  j = 0;
                  for (l in items.fields) {
                    if (items.fields.hasOwnProperty(l)) {
    ...
    jqPivot: function(data, pivotOpt, gridOpt, ajaxOpt) {
      return this.each(function() {
        ...
        if (pivotOpt.loadMsg) {
          $(
            "<div class='loading_pivot ui-state-default ui-state-active row'>" +
              $.jgrid.getRegional(
                $t,
                "regional." + regional + ".defaults.loadtext"
              ) +
              "</div>"
          )
            .insertBefore($t)
        ...
        function pivot(data) {
          ...
          var pivotGrid = jQuery($t).jqGrid("pivotSetup", data, pivotOpt),
            ...
            query = $.jgrid.from.call($t, pivotGrid.rows),
            i,
            so,
            st,
            len;
          ...
          for (
            i = 0;
            i < pivotGrid.groupOptions.groupingView.groupField.length;
            i++
          ) {
            so = pivotOpt.xDimension[i].sortorder
              ? pivotOpt.xDimension[i].sortorder
              : "asc";
            st = pivotOpt.xDimension[i].sorttype
              ? pivotOpt.xDimension[i].sorttype
              : "text";
            query.orderBy(
              pivotGrid.groupOptions.groupingView.groupField[i],
              so,
              st,
              "",
              st
          ...
          if (gridOpt.sortname) {
            // should be a part of xDimension
            ...
            if (pivotGrid.groupOptions.sortname && len) {
              so = pivotOpt.xDimension[len - 1].sortorder
                ? pivotOpt.xDimension[len - 1].sortorder
                : "asc";
              st = pivotOpt.xDimension[len - 1].sorttype
                ? pivotOpt.xDimension[len - 1].sorttype
                : "text";
          ...
          jQuery($t).jqGrid(
            $.extend(
              true,
              {
                datastr: $.extend(
                  query.select(),
                  footerrow ? { userdata: pivotGrid.summary } : {}
                ),
                datatype: "jsonstring",
                footerrow: footerrow,
            ...
            $.extend(
              {
                ...
                success: function(response) {
                  pivot(
                    $.jgrid.getAccessor(
                      response,
                      ajaxOpt && ajaxOpt.reader ? ajaxOpt.reader : "rows"
                    )
                  );
                }
              },
  ...
  $.jgrid.extend({
    ...
    addSubGridCell: function(pos, iRow) {
      ...
      return (
        ...
        prp +
        "><a style='cursor:pointer;' class='ui-sghref'><span class='" +
        icb.icon_base +
        " " +
        ic +
        "'></span></a></td>"
    ...
    addSubGrid: function(pos, sind) {
      return this.each(function() {
        ...
        var subGridCell = function(trdiv, cell, pos) {
          var tddiv = $(
            "<td align='" + ts.p.subGridModel[0].align[pos] + "'></td>"
          ).html(cell);
          $(trdiv).append(tddiv);
        };
        var subGridXml = function(sjxml, sbid) {
          var tddiv,
            ...
            dummy = $(
              "<table class='" +
                base.rowTable +
                " ui-common-table'><tbody></tbody></table>"
            ),
            trdiv = $("<tr></tr>");
          for (i = 0; i < ts.p.subGridModel[0].name.length; i++) {
            tddiv = $(
              "<th class='" +
                base.headerBox +
                " ui-th-subgrid ui-th-column ui-th-" +
                ts.p.direction +
                "'></th>"
            );
          ...
          if (sjxml) {
            ...
            $(sgmap.root + " " + sgmap.row, sjxml).each(function() {
              trdiv = $(
                "<tr class='" + common.content + " ui-subtblcell'></tr>"
              );
              if (sgmap.repeatitems === true) {
                ...
                var f =
                  ts.p.subGridModel[0].mapping || ts.p.subGridModel[0].name;
                if (f) {
                  for (i = 0; i < f.length; i++) {
                    subGridCell(
                      trdiv,
                      $.jgrid.getXmlData(this, f[i]) || "&#160;",
                      i
                    );
                  }
                }
            ...
            for (j = 0; j < ts.p.subGridModel[0].params.length; j++) {
              for (i = 0; i < ts.p.colModel.length; i++) {
                if (ts.p.colModel[i].name === ts.p.subGridModel[0].params[j]) {
                  dp[ts.p.colModel[i].name] = $("td:eq(" + i + ")", rd)
                    .text()
                    .replace(/\&#160\;/gi, "");
                }
              }
              ...
              case "json":
                $.ajax(
                  $.extend(
                    {
                      type: ts.p.mtype,
                      url: $.isFunction(ts.p.subGridUrl)
                        ? ts.p.subGridUrl.call(ts, dp)
                        : ts.p.subGridUrl,
                      dataType: ts.p.subgridtype,
                      data: $.isFunction(ts.p.serializeSubGridData)
                        ? ts.p.serializeSubGridData.call(ts, dp)
                        : dp,
        ...
        var len = ts.rows.length,
          i = 1,
          hsret,
          ishsg = $.isFunction(ts.p.isHasSubGrid);
            ...
            if (hsret === false) {
              ...
              $(ts.rows[i].cells[pos]).on("click", function() {
                ...
                if ($(this).hasClass("sgcollapsed")) {
                  bfsc = $(ts).triggerHandler("jqGridSubGridBeforeExpand", [
                    pID + "_" + _id,
                    _id
                  ]);
                  bfsc = bfsc === false || bfsc === "stop" ? false : true;
                  if (bfsc && $.isFunction(ts.p.subGridBeforeExpand)) {
                    bfsc = ts.p.subGridBeforeExpand.call(
                      ts,
                      pID + "_" + _id,
                      _id
                    );
                  ...
                  if (
                    ts.p.subGridOptions.reloadOnExpand === true ||
                    (ts.p.subGridOptions.reloadOnExpand === false &&
                      !$r.hasClass("ui-subgrid"))
                  ) {
                    atd =
                      pos >= 1 ? "<td colspan='" + pos + "'>&#160;</td>" : "";
                    $(tr).after(
                      "<tr role='row' id='" +
                        pID +
                        "_" +
                        _id +
                        "_expandedContent" +
                        "' class='ui-subgrid ui-sg-expanded'>" +
                        atd +
                        "<td class='" +
                        common.content +
                        " subgrid-cell'><span class='" +
                        common.icon_base +
                        " " +
                        ts.p.subGridOptions.openicon +
                        "'></span></td><td colspan='" +
                        parseInt(ts.p.colNames.length - 1 - nhc, 10) +
                        "' class='" +
                        common.content +
                        " subgrid-data'><div id=" +
                        pID +
                    ...
                    $(ts).triggerHandler("jqGridSubGridRowExpanded", [
                      pID + "_" + _id,
                      _id
                    ]);
                    ...
                    $r.show()
                      .removeClass("ui-sg-collapsed")
                      .addClass("ui-sg-expanded");
                  }
                  $(this)
                    .html(
                      "<a style='cursor:pointer;' class='ui-sghref'><span class='" +
                  ...
                  bfsc = $(ts).triggerHandler("jqGridSubGridRowColapsed", [
                    pID + "_" + _id,
                    _id
                  ]);
                  bfsc = bfsc === false || bfsc === "stop" ? false : true;
                  if (bfsc && $.isFunction(ts.p.subGridRowColapsed)) {
                    bfsc = ts.p.subGridRowColapsed.call(
                      ts,
                      pID + "_" + _id,
                      _id
                    );
                  ...
                  if (ts.p.subGridOptions.reloadOnExpand === true) {
                    $r.remove(".ui-subgrid");
                  } else if ($r.hasClass("ui-subgrid")) {
                    // incase of dynamic deleting
                    $r.hide()
                      .addClass("ui-sg-collapsed")
                      .removeClass("ui-sg-expanded");
                  }
                  $(this)
                    .html(
                      "<a style='cursor:pointer;' class='ui-sghref'><span class='" +
  ...
  $.jgrid.extend({
      ...
      return this.each(function() {
        ...
        while (i < len) {
          ...
          if (ldat[isLeaf] === "true" || ldat[isLeaf] === true) {
            twrap +=
              (ldat[icon] !== undefined && ldat[icon] !== ""
                ? ldat[icon]
                : $t.p.treeIcons.leaf) + " tree-leaf treeclick";
            ldat[isLeaf] = true;
            lf = "leaf";
          } else {
            ldat[isLeaf] = false;
          ...
          ldat[expanded] =
            (ldat[expanded] === "true" || ldat[expanded] === true
              ? true
              : false) &&
          ...
          if (ldat[expanded] === false) {
            twrap +=
              ldat[isLeaf] === true
                ? "'"
                : $t.p.treeIcons.plus + " tree-plus treeclick'";
          } else {
            twrap +=
              ldat[isLeaf] === true
                ? "'"
                : $t.p.treeIcons.minus + " tree-minus treeclick'";
          }

          twrap += "></div></div>";
          $($t.rows[i].cells[expCol])
            .wrapInner("<span class='cell-wrapper" + lf + "'></span>")
            .prepend(twrap);
          ...
          $($t.rows[i].cells[expCol])
            ...
            .on("click", function(e) {
              var target = e.target || e.srcElement,
                ind2 = $.jgrid.stripPref(
                  $t.p.idPrefix,
                  $(target, $t.rows).closest("tr.jqgrow")[0].id
                ),
                pos = $t.p._index[ind2];
    ...
    setTreeGrid: function() {
      return this.each(function() {
        ...
        $t.p.gridview = true;
        //bvn
        if ($t.p.rowTotal === null && !$t.p.treeGrid_bigData) {
          $t.p.rowNum = 10000;
        }
        ...
        $t.p.treeIcons = $.extend(
          { plus: pico, minus: classes.icon_minus, leaf: classes.icon_leaf },
          $t.p.treeIcons || {}
        );
        if ($t.p.treeGridModel === "nested") {
        ...
        for (key in $t.p.colModel) {
          if ($t.p.colModel.hasOwnProperty(key)) {
            ...
            for (tkey in $t.p.treeReader) {
              if (
                $t.p.treeReader.hasOwnProperty(tkey) &&
    ...
    expandRow: function(record) {
      this.each(function() {
        ...
        var childern = $($t).jqGrid("getNodeChildren", record),
          ...
          ret = $($t).triggerHandler("jqGridBeforeExpandTreeGridRow", [
            rowid,
            record,
            childern
          ]);
        ...
        $(childern).each(function() {
          var id =
            $t.p.idPrefix + $.jgrid.getAccessor(this, $t.p.localReader.id);
          $($($t).jqGrid("getGridRowById", id)).css("display", "");
          if (this[expanded]) {
            $($t).jqGrid("expandRow", this);
          }
        });
        $($t).triggerHandler("jqGridAfterExpandTreeGridRow", [
          rowid,
          record,
          childern
        ]);
    ...
    collapseRow: function(record) {
      this.each(function() {
        ...
        var childern = $($t).jqGrid("getNodeChildren", record),
          expanded = $t.p.treeReader.expanded_field,
          rowid = record[$t.p.localReader.id],
          ret = $($t).triggerHandler("jqGridBeforeCollapseTreeGridRow", [
            rowid,
            record,
            childern
          ]);
        ...
        if (ret && $.isFunction($t.p.beforeCollapseTreeGridRow)) {
          ret = $t.p.beforeCollapseTreeGridRow.call(
            $t,
            rowid,
            record,
            childern
        ...
        $($t).triggerHandler("jqGridAfterCollapseTreeGridRow", [
          rowid,
          record,
          childern
        ]);
      ...
      this.each(function() {
        ...
        switch ($t.p.treeGridModel) {
          case "nested":
            ...
            $(view).each(function() {
              if (
                parseInt(this[level], 10) === parseInt($t.p.tree_root_level, 10)
              ) {
                result.push(this);
              }
            });
          ...
          case "adjacency":
            ...
            $(view).each(function() {
              if (
                this[parent_id] === null ||
                String(this[parent_id]).toLowerCase() === "null"
              ) {
                result.push(this);
              }
            });
        ...
        switch ($t.p.treeGridModel) {
          case "nested":
            ...
            $(this.p.data).each(function() {
              if (
                parseInt(this[levelc], 10) === level - 1 &&
                parseInt(this[lftc], 10) < lft &&
                parseInt(this[rgtc], 10) > rgt
              ) {
                result = this;
                return false;
              }
          ...
          case "adjacency":
            ...
            while (pos--) {
              if (
                String($t.p.data[pos][dtid]) ===
                String($.jgrid.stripPref($t.p.idPrefix, rc[parent_id]))
              ) {
                result = $t.p.data[pos];
                break;
              }
        ...
        switch ($t.p.treeGridModel) {
          case "nested":
            ...
            for (i = 0; i < len; i++) {
              ...
              if (
                row &&
                parseInt(row[levelc], 10) === level + 1 &&
                parseInt(row[lftc], 10) > lft &&
                parseInt(row[rgtc], 10) < rgt
              ) {
                result.push(row);
              }
            }
            ...
            for (i = 0; i < len; i++) {
              ...
              if (
                ...
                String(row[parent_id]) ===
                  String($.jgrid.stripPref($t.p.idPrefix, rc[dtid]))
              ) {
                result.push(row);
              }
            }
      ...
      this.each(function() {
        ...
        switch ($t.p.treeGridModel) {
          case "nested":
            ...
            $(this.p.data).each(function() {
              if (
                parseInt(this[levelc], 10) >= level &&
                parseInt(this[lftc], 10) >= lft &&
                parseInt(this[lftc], 10) <= rgt
              ) {
                if (expand) {
                  this[expanded] = true;
                }
            ...
            if (rc) {
              ...
              $(this.p.data).each(function(i) {
                ...
                for (i = 0; i < len; i++) {
                  if (
                    String(
                      $.jgrid.stripPref($t.p.idPrefix, result[i][dtid])
      ...
      this.each(function() {
        ...
        if (rc !== undefined) {
          if (rc[loaded] !== undefined) {
            ...
            rc[isLeaf] ||
            $($t).jqGrid("getNodeChildren", rc).length > 0
          ) {
            result = true;
          } else {
            result = false;
      ...
      return this.each(function() {
        ...
        if (state === true) {
          ...
          $("div.treeclick", rc1)
            ...
            .addClass(this.p.treeIcons.leaf + " tree-leaf");
        } else if (state === false) {
          ...
          $("div.treeclick", rc1)
            .removeClass(this.p.treeIcons.leaf + " tree-leaf")
            .addClass(ico);
        }
      });
    },
    reloadNode: function(rc, reloadcurrent) {
      return this.each(function() {
        ...
        if (this.p.treeGridModel === "nested") {
          ...
          $(this).jqGrid("setGridParam", {
            postData: { nodeid: id, parentid: rc[parent], n_level: rc[level] }
          });
        }
    ...
    expandNode: function(rc) {
      return this.each(function() {
        ...
        if (!rc[expanded]) {
          var id = $.jgrid.getAccessor(rc, this.p.localReader.id),
            rc1 = $(
              "#" + this.p.idPrefix + $.jgrid.jqID(id),
              this.grid.bDiv
            )[0],
            position = this.p._index[id],
            ret = $($t).triggerHandler("jqGridBeforeExpandTreeGridNode", [
              id,
              rc
            ]);
          if (ret === undefined) {
      ...
      return this.each(function() {
        ...
        if (rc[expanded]) {
          var id = $.jgrid.getAccessor(rc, this.p.localReader.id),
            ...
            ret = $($t).triggerHandler("jqGridBeforeCollapseTreeGridNode", [
              id,
              rc
            ]);
          if (ret === undefined) {
    ...
    SortTree: function(sortname, newDir, st, datefmt) {
      return this.each(function() {
        ...
        $.each(records, function(index) {
          ...
          $("#" + $.jgrid.jqID($t.p.id) + " tbody tr:eq(" + index + ")").after(
            $("tr#" + $.jgrid.jqID(id), $t.grid.bDiv)
          );
        });
        query = null;
        roots = null;
      ...
      this.each(function() {
        ...
        if (i) {
          ...
          while (i--) {
            ...
            tid = ancestors[0][lid];
            if ($.inArray(tid, roots) !== -1) {
              // ignore repeated, but add missing
    ...
    collectChildrenSortTree: function(
      records,
      rec,
      sortname,
      newDir,
        ...
        if (nodeid == null) {
          ...
          if (i >= 0) {
            while (i >= 0) {
              max = Math.max(
                max,
                parseInt($t.p.data[i][$t.p.localReader.id], 10)
              );
              i--;
            }
          }
        ...
        if (parentid === undefined || parentid === null || parentid === "") {
          ...
          if (parentdata[isLeaf]) {
            ...
            $($t.rows[prow])
              ...
              .end()
              .find("div.tree-leaf")
              .removeClass($t.p.treeIcons.leaf + " tree-leaf")
              .addClass($t.p.treeIcons.minus + " tree-minus");
            $t.p.data[parentindex][isLeaf] = false;
            parentdata[loaded] = true;
          }
          // incremet th index of child to be inserted
          if (i === false) {
            throw "Parent item with id: " +
              rowind +
              " (" +
              parentid +
              ") can't be found";
          ...
          if (parentid !== null) {
            ...
            if (res.length) {
              for (key in res) {
                if (res.hasOwnProperty(key)) {
                  res[key][left] =
                    res[key][left] > maxright
                      ? parseInt(res[key][left], 10) + 2
                      : res[key][left];
                  res[key][right] =
                    res[key][right] >= maxright
                      ? parseInt(res[key][right], 10) + 2
                      : res[key][right];
                }
              }
            }
            ...
            maxright = parseInt(
              $($t).jqGrid("getCol", right, false, "max"),
              10
            );
            res = $.jgrid.from
              .call($t, $t.p.data)
...
*/
    ...
    return this.each(function() {
      ...
      handle.on({
        mousedown: function(e) {
          if (
            settings.exclude &&
            ~$.inArray(e.target, $(settings.exclude, el))
          ) {
            return;
          }
          e.preventDefault();
  ...
  $.jqm = {
    ...
    open: function(s, t) {
      ...
      if (c.toTop && h.o) {
        h.w
          .before('<span id="jqmP' + h.w[0]._jqm + '"></span>')
          .insertAfter(h.o);
      }
  ...
  var s = 0,
    ...
    f = function(h) {
      try {
        $(":input:visible", h.w)[parseInt(h.c.focusField, 10)].focus();
      } catch (_) {}
    },
    L = function(t) {
      $(document)
    ...
    NumberFormat: function(nData, opts) {
      ...
      if ($.fmatter.isNumber(nData)) {
        ...
        if ($.fmatter.isNumber(opts.decimalPlaces)) {
          ...
          sOutput = String(
            Number(
              Math.round(nData + "e" + nDecimalPlaces) + "e-" + nDecimalPlaces
            )
          );
  ...
  $.fn.fmatter.checkbox = function(cval, opts) {
    var op = $.extend({}, opts.checkbox),
      ds;
    if (
      opts.colModel !== undefined &&
      opts.colModel.formatoptions !== undefined
    ) {
    ...
    var bchk =
      cval.search(/(false|f|0|no|n|off|undefined)/i) < 0
        ? " checked='checked' "
        : "";
    return (
      '<input type="checkbox" ' +
  ...
  $.fn.fmatter.showlink = function(cellval, opts) {
    ...
    if ($.fmatter.isString(cellval) || $.fmatter.isNumber(cellval)) {
      //add this one even if its blank string
  ...
  $.fn.fmatter.select = function(cellval, opts) {
    ...
    if (opts.colModel.formatoptions !== undefined) {
      ...
      sep =
        opts.colModel.formatoptions.separator === undefined
          ? ":"
          : opts.colModel.formatoptions.separator;
      delim =
        opts.colModel.formatoptions.delimiter === undefined
          ? ";"
          : opts.colModel.formatoptions.delimiter;
    } else if (opts.colModel.editoptions !== undefined) {
      oSelect = opts.colModel.editoptions.value;
      sep =
        opts.colModel.editoptions.separator === undefined
          ? ":"
          : opts.colModel.editoptions.separator;
      delim =
        opts.colModel.editoptions.delimiter === undefined
          ? ";"
          : opts.colModel.editoptions.delimiter;
    }
    if (oSelect) {
      var msl =
          (opts.colModel.editoptions != null &&
            opts.colModel.editoptions.multiple === true) === true
            ? true
            : false,
        scell = [],
        sv;
  ...
  $.fn.fmatter.rowactions = function(act) {
    var $tr = $(this).closest("tr.jqgrow"),
      ...
      $actionsDiv = cm.frozen
        ? $(
            "tr#" + rid + " td:eq(" + $.jgrid.getCellIndex(this) + ") > div",
            $grid
          )
        : $(this).parent(),
      op = {
  ...
  $.fn.fmatter.actions = function(cellval, opts) {
    ...
    var hover =
      "onmouseover=jQuery(this).addClass('" +
      common.hover +
      "'); onmouseout=jQuery(this).removeClass('" +
      common.hover +
      "');  ";
    if (op.editformbutton) {
      ocl =
        "id='jEditButton_" +
        rowid +
        "' onclick=jQuery.fn.fmatter.rowactions.call(this,'formedit'); " +
        hover;
      str +=
        "<div title='" +
        nav.edittitle +
    ...
    if (op.delbutton) {
      ...
      str +=
        "<div title='" +
        nav.deltitle +
        "' style='float:left;' class='ui-pg-div ui-inline-del' " +
        ocl +
        "><span class='" +
        common.icon_base +
        " " +
    ...
    str +=
      "<div title='" +
      nav.canceltitle +
      "' style='float:left;display:none;' class='ui-pg-div ui-inline-cancel' " +
      ocl +
      "><span class='" +
      common.icon_base +
      " " +
  ...
  $.unformat = function(cellval, options, pos, cnt) {
    ...
    var ret,
      formatType = options.colModel.formatter,
      op = options.colModel.formatoptions || {},
      sep,
      re = /([\.\*\_\'\(\)\{\}\+\?\\])/g,
      unformatFunc =
        options.colModel.unformat ||
  ...
  $.unformat.select = function(cellval, options, pos, cnt) {
    ...
    var op = $.extend(
        {},
        options.colModel.formatoptions !== undefined
  ...
  $.extend($.jgrid, {
    ...
    encode: function(text) {
      // repeated, but should not depend on grid
      return String(text)
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
        ...
        scalarToxml = function(name, text) {
          ...
          if (text === "") {
            ...
            return (
              ...
              name +
              ">" +
              (o.encode ? that.encode(text) : text) +
              "</" +
              name +
              ">\n"
            );
  ...
  $.extend($.jgrid, {
    ...
    formatCellCsv: function(v, p) {
      ...
      try {
        v = v
          .replace(p._regexsep, p.separatorReplace)
          .replace(/\r\n/g, p.replaceNewLine)
          .replace(/\n/g, p.replaceNewLine);
      } catch (_e) {
        v = "";
      ...
      this.each(function() {
            ...
            for (k = foffset; k < colspans; k++) {
              ...
              $.each(fdata.summary, function() {
                if (this.nm === cm[k].name) {
                  ...
                  retarr[j] = $.jgrid.formatCellCsv(
                    $.jgrid.stripHtml($.jgrid.template(tplfld, vv)),
                    p
                  );
                  return false;
                }
          ...
          $.each(grp.groups, function(i, n) {
            ...
            if (grp.groupSummaryPos[n.idx] === "header") {
              arr = buildSummaryTd(
                ...
                0,
                grp.groups,
                0 /*grp.groupColumnShow[n.idx] === false ? (mul ==="" ? 2 : 3) : ((mul ==="") ? 1 : 2)*/
              );
            } else {
        ...
        if ($.isFunction(p.loadIndicator)) {
          ...
          $($t).jqGrid("progressBar", {
            method: "show",
            loadtype: $t.p.loadui,
            htmlcontent: $.jgrid.getRegional($t, "defaults.loadtext")
          });
        }
        // end group function
        var def = [],
        ...
        if ($t.p.grouping) {
          ...
          while (j < dlen) {
            ...
            for (i = 0; i < cmlen; i++) {
              if (cm[i]._expcol) {
                tmp[k] = $.jgrid.formatCellCsv(
                  $.jgrid.formatCell(row[cm[i].name], i, row, cm[i], $t, "csv"),
                  p
                );
                k++;
              }
        ...
        if (
          p.includeGroupHeader &&
          $t.p.groupHeader &&
          $t.p.groupHeader.length
        ) {
          ...
          if (foot.length) {
            ...
            while (i < p.collen) {
              ...
              if (frows.hasOwnProperty(fc)) {
                tmp.push(
                  $.jgrid.formatCellCsv($.jgrid.stripHtml(frows[fc]), p)
                );
              }
              i++;
            }
    ...
    exportToExcel: function(o) {
      ...
      this.each(function() {
        ...
        var addStyle = function(eo) {
          if ($.isEmptyObject(eo)) {
            ...
            $.each(fmnt, function(i, n) {
              ...
              maxfmtid = Math.max(
                maxfmtid,
                parseInt($(n).attr("numFmtId"), 10)
              );
            });
            var mycell = $.jgrid.makeNode(styleSh, "numFmt", {
              attr: { numFmtId: maxfmtid + 1, formatCode: eo.excel_format }
            });
            formats.appendChild(mycell);
            $(formats).attr("count", count + 1);
            count = 0;
            mycell = $.jgrid.makeNode(styleSh, "xf", {
              attr: {
                numFmtId: maxfmtid + 1 + "",
                fontId: "0",
                fillId: "0",
        ...
        for (j = 0, ien = cm.length; j < ien; j++) {
          ...
          data.width[i] = 5;
          data.map[i] = j;
          data.parser[j] = addStyle(
            cm[j].hasOwnProperty("exportoptions") ? cm[j].exportoptions : {}
          );
          i++;
        }
        ...
        var _replStr = $.isFunction(o.replaceStr) ? o.replaceStr : _replStrFunc,
          ...
          addRow = function(row, header) {
            currentRow = rowPos + 1;
            rowNode = $.jgrid.makeNode(rels, "row", {
              attr: { r: currentRow }
            });
            var maxieenum = 15,
              text;
            for (var i = 0; i < data.header.length; i++) {
              ...
              data.width[i] = Math.max(
                data.width[i],
                Math.min(parseInt(v.toString().length, 10), o.maxlength)
              );
              cell = null;
              var expo = data.parser[data.map[i]];
              if (expo.excel_parsers === true && !header) {
                for (
                  var j = 0, jen = $.jgrid.excelParsers.length;
                  j < jen;
                  j++
                ) {
                  ...
                  if (v.match && !v.match(/^0\d+/) && v.match(special.match)) {
                    ...
                    if (special.style === 67) {
                      //Dates
                      cell = _makeCellSpecial(
                        { t: "d", r: cellId, s: special.style },
                        v
                      );
                    } else {
                      if ($.inArray(special.style, ["63", "64", "65", "66"])) {
                        // Numbers
                ...
                expo.excel_format !== undefined &&
                expo.excel_style !== undefined &&
                !header &&
                !cell
              ) {
              ...
              if (!cell) {
                ...
                if (
                  (typeof v === "number" && v.toString().length <= maxieenum) ||
                  (match &&
                    match[1].length + (match[2] ? match[3].length : 0) <=
                      maxieenum)
                ) {
        ...
        function groupToExcel(grdata) {
            ...
            for (k = foffset; k < colspans; k++) {
              ...
              $.each(fdata.summary, function() {
                if (this.nm === cm[k].name) {
                  ...
                  retarr[this.nm] = $.jgrid.stripHtml(
          ...
          $.each(grp.groups, function(i, n) {
            ...
            arr[fkey[0]] = $.jgrid.stripHtml(
              new Array(n.idx * 5).join(" ") + grpTextStr
            );
            addRow(arr, true);
            var leaf = len - 1 === n.idx;
            if (leaf) {
              ...
              for (var k = 0; k < ghdata.length; k++) {
                if (ghdata[k].startColumnName === key) {
                  ...
                  var start = $.jgrid.excelCellPos(j) + mrow,
                      ...
                      $.jgrid.excelCellPos(j + ghdata[k].numberOfColumns - 1) +
                      mrow;
        ...
        try {
          ...
          if (zip.generateAsync) {
            ...
            // JSZip 2.5
            $.jgrid.saveAs(zip.generate(zipConfig), o.fileName, {
              type: o.mimetype
            });
          }
        } catch (e) {
  ...
  $.jgrid.extend({
    ariaBodyGrid: function(p) {
      ...
      return this.each(function() {
        ...
        $($t).on("jqGridAfterGridComplete.setAriaGrid", function(e) {
          //var grid = e.target;
          $(
            "tbody:first>tr:not(.jqgfirstrow)>td:not(:hidden, :has(" +
        ...
        var focusRow = 0,
          focusCol = 0; // set the dafualt one
        ...
        $($t).on("jqGridCellSelect.ariaGridClick", function(
          el1,
          id,
          status,
          tdhtml,
    ...
    focusBodyCell: function(focusRow, focusCol, _s, _h) {
      return this.each(function() {
        var $t = this,
          ...
          highlight = !_h
            ? getstyle($t.p.styleUI + ".common", "highlight", true)
            : _h,
    ...
    ariaHeaderGrid: function() {
      return this.each(function() {
          ...
          switch (key) {
            case 37: // left
              if (currindex - 1 >= 0) {
                currindex--;
                while (
                  $($t.grid.headers[currindex].el).is(":hidden") &&
                  currindex - 1 >= 0
                ) {
                  currindex--;
                  if (currindex < 0) {
        ...
        $("tr.ui-jqgrid-labels>th:not(:hidden)", htable)
          .attr("tabindex", -1)
          .focus(function() {
            $(this)
              .addClass(highlight)
    ...
    ariaPagerGrid: function() {
      return this.each(function() {
        ...
        $("table.ui-pager-table tr:first", $t.p.pager).on("keydown", function(
          e
        ) {
          var key = e.which || e.keyCode;
...
(function($) {
  function colapsarTodo() {
    do {
      if (colapsado)
        listaSpans = document.getElementsByClassName(
